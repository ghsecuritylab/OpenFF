...
#if defined(CONFIG_SILENT_CONSOLE) || defined(ETHERNET_INIT_DEFER)
...
// Added by wayne on 12/21/2009
int iTestLEDOff=1;	// 1 means OFF
...
# ifdef CONFIG_AUTOBOOT_KEYED
		int prev = disable_ctrlc(1);	/* disable Control C checking */
# endif
		firmware_tftp_recover();		
#ifdef CFG_NMRP
		StartNmrpClient();
#endif
		// Added by wayne on 10/26/2009
		// Light on LAN LED if PHY in 10 MBps
//		ag7240_scan_link();
