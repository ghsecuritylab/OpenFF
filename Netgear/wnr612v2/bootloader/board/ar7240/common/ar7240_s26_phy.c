...
void athrs26_reg_init(void)
{
    ...
    if(ar7240_revid == AR7240_REV_1_0) {	// wayne: It's AR7240_REV_1_2
		...
		// Modified by wayne on 2009/11/09
		// PHY soft reset will be done in athrs26_phy_setup
...
athrs26_phy_setup(int ethUnit)
{
     ...
     */
     ...
     {
        sysMsDelay(500);
 	 wlan_led(1);		// OFF WAN LED
        sysMsDelay(500);
    }
     else
     {
		// Modified by wayne on 2009/11/09
		// Let's leave the rest of work to the LAN unit
	 	return FALSE;
        sysMsDelay(3000);
     }
     ...
     */
     // Modified by wayne on 2009/11/05
     // Don't bother to check for status of all ports. let's do it only for the last port of LAN
        ...
        if (lastStatus->isPhyAlive) { /* last known link status was ALIVE */
             ...
             */
                ...
                DRV_PRINT(DRV_DEBUG_PHYCHANGE,("\nenet%d port%d down\n",
                                               ethUnit, phyUnit));
				// Added by wayne lu on 2009/11/02
				// Note that the port with 100Mpbs is not affected by the following statement
				// It means only 10Mpbs is affected by the manual LED command.
				s26_wr_phy(phyUnit,0x19,0); /* add by alfa */
            ...
            if ((!(phyHwControl & ATHR_CTRL_AUTONEGOTIATION_ENABLE)) 
                 || ATHR_AUTONEG_DONE(phyHwStatus)) {
                    ...
                    if (phyHwStatus & ATHR_STATUS_LINK_PASS) {
                        ...
                        DRV_PRINT(DRV_DEBUG_PHYCHANGE,("\nenet%d port%d up\n",
                                               ethUnit, phyUnit));
						// Added by wayne on 2009/11/02
						// Note that the port with 100Mpbs is not affected by the following statement
						// It means only 10Mpbs is affected by the menual LED command.
...
// Added by wayne on 10/26/2009
static int athrs26_phy_speed_ext(int phyUnit)
{
    uint16_t  phyHwStatus;
    uint32_t  phyBase;
    uint32_t  phyAddr;
...
uint32_t      phyBase;
uint32_t      phyAddr;
static int	iPhySpeed[ATHR_PHY_MAX];
int iLastPhySpeed;
	...
	for (phyUnit=0; phyUnit < ATHR_PHY_MAX; phyUnit++) {
		...
		if (lastStatus->isPhyAlive) { /* last known link status was ALIVE */
			...
			iLastPhySpeed = iPhySpeed[phyUnit];
			iPhySpeed[phyUnit] = athrs26_phy_speed_ext(phyUnit);
			if(iPhySpeed[phyUnit] == _10BASET)
			{
				s26_wr_phy(phyUnit,0x19,0x3c0);
			}
			else if(iPhySpeed[phyUnit] ==-1)
			{
				if(iLastPhySpeed==_10BASET)
					s26_wr_phy(phyUnit,0x19,0);
				lastStatus->isPhyAlive = FALSE;
			}
		} else { /* last known link status was DEAD */
