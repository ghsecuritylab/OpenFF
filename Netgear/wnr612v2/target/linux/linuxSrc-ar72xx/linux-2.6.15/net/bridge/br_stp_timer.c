 ...
 *
 *	$Id: //depot/sw/releases/7.3_AP/linux/kernels/mips-linux-2.6.15/net/bridge/br_stp_timer.c#1 $
 *
...
#ifdef IPTV
#define	IPTV_IGMP_CHECK_GCPERIOD	(10*HZ)
extern struct br_timer	iptv_igmp_timer;
...
static void br_check_timers(struct net_bridge *br)
{
	...
	if( iptv_on == 1 && strcmp(cBrgName,"br1") == 0 )
	{
		if (br_timer_has_expired(&iptv_igmp_timer, IPTV_IGMP_CHECK_GCPERIOD)) {
			br_timer_set(&iptv_igmp_timer, jiffies);
			iptv_igmp_gc();
		}
	}
...
#ifdef IPTV
	br_timer_init(&br->gc_timer, br_check_timers, (unsigned long) br);
#endif
}
