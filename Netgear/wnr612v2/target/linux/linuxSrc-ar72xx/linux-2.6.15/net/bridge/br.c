...
#endif
#ifdef IPTV
struct proc_dir_entry *igmp;
unsigned int igmp_ready=1;
static char igmp_setting[4];
extern void br_igmpv3_init(void);
#endif
...
//Added by Jonathan
static int igmp_ready_write_proc(struct file *file, const char *buffer,
		      unsigned long count, void *data)
{
	printk("igmp_ready_write_proc \n");

	if (buffer && !copy_from_user(igmp_setting, buffer, count)) {
        igmp_setting[count-1] = 0; // remove 0x0a
        if (igmp_setting[0] == '0')
        	igmp_ready = 0;
        else
        	igmp_ready = 1;
...
//Added by Jonathan
static int igmp_ready_read_proc(char *page, char **start, off_t off,
		     int count, int *eof, void *data)
{
	printk("%d\n", igmp_ready);
	return 0;
...
#ifdef IPTV	
	//when iptv.sh is finish, set igmp_ready to 1. 
	//igmp_ready is 1, the streaming will transmit to clients	
	igmp = create_proc_entry("igmp_ready", 0, NULL);
	igmp->read_proc = igmp_ready_read_proc;
	igmp->write_proc = igmp_ready_write_proc;
#endif
}
