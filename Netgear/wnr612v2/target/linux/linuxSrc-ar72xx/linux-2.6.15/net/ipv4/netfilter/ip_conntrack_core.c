...
#ifdef CONFIG_IP_NF_CT_SCAN
#define LAN_IFNAME "br0"
#define WAN_PPP_IFNAME "ppp0"
#define WAN_IPC_IFNAME "eth0" // eth0 for WNR612 project
#endif
...
atomic_t ip_conntrack_icmp_reserve_count = ATOMIC_INIT(0);
atomic_t ip_conntrack_tcp_reserve_count = ATOMIC_INIT(0);
#endif
...
int ip_conntrack_max = 4096;
#if defined(CONFIG_IP_NF_CT_NAT_RESERVE)
int ip_conntrack_icmp_reserve_max = 32;
int ip_conntrack_tcp_reserve_max = 480;
#endif
...
unsigned int p2p_session_max = 200;
unsigned int p2p_session_count = 0;
extern int gQosEnabled;
#endif

#ifdef CONFIG_DNI_NAT_SESS_MGNT
int (*session_mgnt_hook1)(unsigned long) = NULL;
EXPORT_SYMBOL(session_mgnt_hook1);
int (*session_mgnt_hook2)(unsigned long, void *) = NULL;
EXPORT_SYMBOL(session_mgnt_hook2);
int (*session_mgnt_hook3)(unsigned long) = NULL;
EXPORT_SYMBOL(session_mgnt_hook3);
#endif
		...
		const struct ip_conntrack_protocol *protocol)
{
	...
	u32_set(&tuple->src.ip, &iph->saddr);
	u32_set(&tuple->dst.ip, &iph->daddr);
#else
...
#ifdef CONFIG_DNI_LIMIT_P2P_SESSION
		if (((ct->status & IPS_P2P_SESSION) == IPS_P2P_SESSION) && p2p_session_count )
			p2p_session_count--;
...
#ifdef CONFIG_DNI_NAT_SESS_MGNT
		if (session_mgnt_hook3)
			session_mgnt_hook3(ct);
#endif
...
//tim11
int session_full_mgnt(struct sk_buff *skb)
{
    
          //printk("##### session_full_mgnt #####\n");
...
__ip_conntrack_confirm(struct sk_buff **pskb)
{
	...
	if (!LIST_FIND(&ip_conntrack_hash[hash],
		 ...
		 *
		 * 1.1	Basic NAT
		 *    4. An implementation SHOULD support up to 2048 concurrent dynamic NAT 
		 * sessions (i.e. besides Port Forwarding and Port Triggering). However, for a 
		 * product which has limited memory issue, it MAY support up to 1024 concurrent 
		 * dynamic NAT sessions (NOTE: products supporting less than 2048 concurrent 
		 * sessions cannot pass some certifications like Skype). The number of sessions 
		 * required here (i.e. 2048/1024) does not include the traffic (or connections) 
		 * destined/initiated to/from the router. For example, a TCP session from a LAN 
		 * PC to the router and UDP traffic initiated from the router to another host on the
		 * Internet are not counted in the total NAT sessions that an implementation has 
		 * to support.
		 *
		 *    5. When the NAT mapping table is full, if there is a new connection initiated, 
		 * the new connection should be blocked. So a new connection cannot pass 
		 * through until there is a NAT mapping entry freed (e.g. idle timeouts, explicit TCP 
		 * disconnection, ...). A new connection MUST NOT kick out an existing active 
		 * session and get an entry to pass through the router.
		 *
		 * 
		 ...
		 * }
		 *
		 * If both IPS_SRC_NAT and IPS_DST_NAT are NOT set, this traffic must be 
		 * destined/initiated to/from the Router, let it be PASSED!!!.
		 */
...
//int cur_sess_num= atomic_read(&ip_conntrack_count);
//printk("######session num=%d, prot= %d, max=%d########\n",cur_sess_num,(*pskb)->nh.iph->protocol,ip_conntrack_max);

if(atomic_read(&ip_conntrack_count) > ip_conntrack_max-200)
{
       if(!session_full_mgnt(*pskb))
       {
...
////
			...
			{
#if defined(CONFIG_IP_NF_CT_NAT_RESERVE)
				if ((ct->status & IPS_CT_ICMP_RESERVE) && ip_conntrack_icmp_reserve_max && atomic_read(&ip_conntrack_icmp_reserve_count) < ip_conntrack_icmp_reserve_max)
						atomic_inc(&ip_conntrack_icmp_reserve_count);
				
/*				else if ((ct->status & IPS_CT_TCP_RESERVE)
					&& ip_conntrack_tcp_reserve_max
					&& atomic_read(&ip_conntrack_tcp_reserve_count) < ip_conntrack_tcp_reserve_max)
						atomic_inc(&ip_conntrack_tcp_reserve_count);
*/
				else 
				{
#endif
				       /* clean the flag value for counting correctly when freed. */
				       ct->status &= ~IPS_NAT_MASK;
		...
		{
			if (ct->status & IPS_CT_TCP_RESERVE)
			{
				if (ip_conntrack_tcp_reserve_max
					&& atomic_read(&ip_conntrack_tcp_reserve_count) < ip_conntrack_tcp_reserve_max)
					atomic_inc(&ip_conntrack_tcp_reserve_count);
				else
				{
...
#endif // #if defined(CONFIG_IP_NF_CT_NAT_RESERVE)
#endif // #if defined(CONFIG_IP_NF_CT_NAT_SESSION)
...
#ifdef CONFIG_DNI_NAT_SESS_MGNT
		if(session_mgnt_hook2)
			session_mgnt_hook2(ct, NULL);
#endif
		write_unlock_bh(&ip_conntrack_lock);
		if (ct->helper)
...
#ifndef CONFIG_IP_NF_CT_NAT_SESSION
...
ip_conntrack_free(struct ip_conntrack *conntrack)
{
	...
	{
		if (is_confirmed(conntrack))
		{
#if defined(CONFIG_IP_NF_CT_NAT_RESERVE)
			if (conntrack->status & IPS_CT_ICMP_RESERVE)
				atomic_dec(&ip_conntrack_icmp_reserve_count);
			else if (conntrack->status & IPS_CT_TCP_RESERVE)
				atomic_dec(&ip_conntrack_tcp_reserve_count);
			else
	...
	else
	{
		if (is_confirmed(conntrack))
		{
			if (conntrack->status & IPS_CT_TCP_RESERVE)
				atomic_dec(&ip_conntrack_tcp_reserve_count);
		}
	}
#endif
	       ...
	       struct sk_buff *skb)
{
	...
	if (ip_conntrack_max
		&& atomic_read(&ip_conntrack_count) >= ip_conntrack_max)
	{
		if (session_mgnt_hook1&&session_mgnt_hook1(skb))
			return NULL;
	}
#endif
	...
	if (exp) {
		...
		strncpy(conntrack->nat.masq_ifname, exp->master->nat.masq_ifname, IFNAMSIZ);
#endif
...
/* Doesn't cover locally-generated broadcast, so not worth it. */
	...
	/* ICMP is an special packet, it needs to invoke `icmp_error`. */
	if (proto->proto == IPPROTO_ICMP && proto->error != NULL 
...
#if defined(CONFIG_DNI_LIMIT_P2P_SESSION) || defined(CONFIG_DNI_NAT_SESS_MGNT)
void call_death_by_timeout(unsigned long l)
{
	struct ip_conntrack *ct = (struct ip_conntrack *)l;
	if (!l)
		return;
	if (del_timer(&ct->timeout))
		death_by_timeout(l);
...
EXPORT_SYMBOL(call_death_by_timeout);
#endif
