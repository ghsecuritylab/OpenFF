          ...
          int (*okfn)(struct sk_buff *))
{
	...
	u32_set(&saddr, &(*pskb)->nh.iph->saddr);
	u32_set(&daddr, &(*pskb)->nh.iph->daddr);

	ret = ip_nat_fn(hooknum, pskb, in, out, okfn);
	if (ret != NF_DROP && ret != NF_STOLEN) {
		u_int32_t new_saddr, new_daddr;
		u32_set(&new_saddr, &(*pskb)->nh.iph->saddr);
		u32_set(&new_daddr, &(*pskb)->nh.iph->daddr);
		if (new_saddr != saddr || new_daddr != daddr) {
