				    ...
				    void *userinfo)
{
	...
	/* Add by yuecheng, for delete unused conntrack  */
	if (mr->range[0].flags & IPS_FORWARDING) {
		DEBUGP("%s: dnat-type: forwarding\n", __FUNCTION__);
		ct->status |= IPS_FORWARDING;
	} else if (mr->range[0].flags & IPS_UPNP) {
		DEBUGP("%s: dnat-type: upnp\n", __FUNCTION__);
		ct->status |= IPS_UPNP;
	} else if (mr->range[0].flags & IPS_DMZ) {
		DEBUGP("%s: dnat-type: dmz\n");
		ct->status |= IPS_DMZ;
	}
#endif
	...
	printk("<7>[LAN access from remote] from %u.%u.%u.%u:%d to %u.%u.%u.%u:%d,\n", 
		NIPQUAD(iph->saddr),	ntohs(tcph->source), NIPQUAD(mr->range[0].min_ip),ntohs(tcph->dest));
