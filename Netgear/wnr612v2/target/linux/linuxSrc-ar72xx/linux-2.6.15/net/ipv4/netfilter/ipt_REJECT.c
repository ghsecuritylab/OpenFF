 ...
 * Added NETGEAR HTTP Blocking and Traffic limit Page
 */
...
static void send_block_site_page(struct sk_buff *oldskb, int hook)
{
	...
	hh_len = LL_RESERVED_SPACE(rt->u.dst.dev);
	nskb = skb_copy_expand(oldskb, hh_len, 835, GFP_ATOMIC);
	...
	tcph->doff = sizeof(struct tcphdr)/4;
	skb_trim(nskb, nskb->nh.iph->ihl*4 + sizeof(struct tcphdr));
	nskb->nh.iph->tot_len = htons(nskb->len +815);		
	needs_ack = 1;
	tcph->seq = oth->ack_seq;
	tcph->ack_seq = htonl(ntohl(oth->seq) + oth->syn + oth->fin 
				+ tcplen - (oth->doff<<2));
	...
	data = skb_put(nskb , 815);
	memcpy(data,web_block_page, 815);

	/* Adjust TCP checksum */
	tcph->check = 0;
	tcph->check = tcp_v4_check(tcph, sizeof(struct tcphdr)+815,
				   nskb->nh.iph->saddr,
				   nskb->nh.iph->daddr,
				   csum_partial((char *)tcph,
...
static void send_traffic_limit_page(struct sk_buff *oldskb, int hook)
{
	...
	hh_len = LL_RESERVED_SPACE(rt->u.dst.dev);
	nskb = skb_copy_expand(oldskb, hh_len, 839, GFP_ATOMIC);
	...
	tcph->ack_seq = htonl(ntohl(oth->seq) + oth->syn + oth->fin 
	 ...
	 /* Reset flags */
        ...
        data = skb_put(nskb , 819);
        memcpy(data,web_block_page, 819);

	/* Adjust TCP checksum */
	tcph->check = 0;
	tcph->check = tcp_v4_check(tcph, sizeof(struct tcphdr)+819,
				   nskb->nh.iph->saddr,
				   nskb->nh.iph->daddr,
				   csum_partial((char *)tcph,
			   ...
			   void *userinfo)
{
	...
	case IPT_HTTP_SITE_PROHIBITED:
		send_block_site_page(*pskb, hooknum);
		send_fin_ack(*pskb , hooknum);
		notify_sending_log_email();
		break;
	case IPT_TRAFFIC_LIMIT:
		send_traffic_limit_page(*pskb, hooknum);
		send_fin_ack(*pskb, hooknum);
		break;
	}
