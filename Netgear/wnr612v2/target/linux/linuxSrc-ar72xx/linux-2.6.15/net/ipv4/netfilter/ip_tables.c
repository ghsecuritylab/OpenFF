...
int ip_ct_tcp_forward_accept = 1;
	     ...
	     void *userdata)
{
	...
	if (ip_ct_tcp_forward_accept == 1 && ((*pskb)->nh.iph->protocol == IPPROTO_TCP) && (hook == NF_IP_FORWARD)) {
	  ...
	  if (ct && (ctinfo == IP_CT_ESTABLISHED
	    ...
	    //            printk("ipt_do_table hooknum=%d: <%u.%u.%u.%u --> %u.%u.%u.%u>\n",       
	    //                   hook,                                                             
	    ...
	    unsigned int *i)
{
	...
	if (strncmp(m->u.user.name, "multiport", 9) == 0)
	  m->u.user.revision = 1;
...
do_replace(void __user *user, unsigned int len)
{
	...
	/*
	 * The initialized number of entries in nat table is *4*; if > *4*, firewall has
	 * updated new entries to do SNAT & DNAT. if firewall is stopped, the rules
	 * number will be back to *4*. According to this number *4*, it deduces that
	 * whether the firewall is restarted or not. If it is restarted, it need to remove
	 * the invalid conntracks with old SNATed Router-WAN-Address.
	 */
	extern struct tasklet_struct nat_tasklet;
	if (strcmp(tmp.name, "nat") == 0 && tmp.num_entries > 4)
		tasklet_schedule(&nat_tasklet);
#endif
	vfree(counters);
	up(&ipt_mutex);
	return ret;
