...
//MODULE_DESCRIPTION("DNISIP connection tracking helper");
...
struct sip_header_nfo ct_sip_hdrs[] = {
	...
	[POS_CONTENT] = { 	/* Content length header */
	...
	[POS_CALL_ID] = { 	/* SDP version header */
		.lname		= "Call-ID:",
		.lnlen		= sizeof("Call-ID:") - 1,
		.sname		= "\r\ni:",
		.snlen		= sizeof("\r\ni:") - 1,
		.ln_str		= "@",
		.ln_strlen	= sizeof("@") - 1,
...
EXPORT_SYMBOL_GPL(parse_ipaddr);
...
static int skp_epaddr_len(const char *dptr, const char *limit, int *shift)
{
	int s = *shift;
	const char *aux = dptr;
	for (; dptr <= limit && *dptr != '@' && *dptr != '\r'; dptr++)
		(*shift)++;
		    ...
		    struct sip_header_nfo *hnfo)
{
	...
	while (dptr <= limit) {
		...
		*matchlen = hnfo->match_len(aux, aux + len, &shift);
		if (!*matchlen)
			return -1;
			    ...
			    const char *dptr)
{
	...
	DEBUGP("set_expected_rtp (sip_help) new conntrack expect\n");
	DEBUGP("tuple: "); DUMP_TUPLE(&exp->tuple);
	DEBUGP("mask:  "); DUMP_TUPLE(&exp->mask);

	if (ip_nat_sdp_hook)
		ret = ip_nat_sdp_hook(pskb, ctinfo, ct, exp, dptr);
	else {
		    ...
		    enum ip_conntrack_info ctinfo)
{
	...
	if (ip_nat_sip_hook) {
		if (!ip_nat_sip_hook(pskb, ctinfo, ct, &dptr)) {
			dptr = (*pskb)->data + dataoff;
			if (ct_sip_search("SIP/2.0", dptr, sizeof("SIP/2.0") - 1, (*pskb)->len - dataoff))
				ret = NF_DROP;
			goto out;
		}
	}
	...
	if (dir == IP_CT_DIR_REPLY) {
		if (ip_nat_sdp_hook)
			ret = ip_nat_sdp_hook(pskb, ctinfo, ct, NULL, dptr);
	...
	if (ct_sip_get_info(dptr, datalen, &matchoff, &matchlen,
		...
		if (ct_sip_get_info(dptr, datalen, &matchoff, &matchlen,
			...
			ret = set_expected_rtp(pskb, ct, ctinfo, ipaddr, port, dptr);
		}
	}
out:
	return ret;
...
#if (DNI_SIP_ALG_PATCH)
	// regoster port 5060 and 5061
	if (ports_c == 1)
		ports[ports_c++] = SIP_PORT + 1;
#endif
