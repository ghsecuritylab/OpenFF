...
/* NETGEAR SPEC, LAN port reservation */
#if defined(CONFIG_IP_NF_TARGET_SNATP2P) && defined(CONFIG_LAN_PORT_RESERVED)
#include <linux/inet.h>
int lan_port_reserved;
struct lan_port_resv_t lan_port_resv[LAN_PORT_RESERVATION_SIZE];
EXPORT_SYMBOL(lan_port_resv);
EXPORT_SYMBOL(lan_port_reserved);
#endif
...
#if defined(CONFIG_IP_NF_TARGET_SNATP2P) && defined(CONFIG_LAN_PORT_RESERVED)
// for kernel 2.4
//	if ((lan_port_reserved > 0) && !(ignored_conntrack->status & IPS_SNATP2P_SRC) && (ignored_conntrack->status & IPS_LAN_IN)) {
// for kernel 2.6.15
	if ((lan_port_reserved > 0) && (ignored_conntrack->status & IPS_SNATP2P_SRC) && (ignored_conntrack->status & IPS_LAN_IN)) {
		int i;
		for (i = 0; i < lan_port_reserved; i++) {
// for kernel 2.4
//			if (!(tuple->src.ip == lan_port_resv[i].ip)) {
// for kernel 2.6.15
			if ((lan_port_resv[i].ip == TRIGGER_ANY_IP) || !(ignored_conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.ip == lan_port_resv[i].ip)) {
				if ((lan_port_resv[i].proto == 255) || (lan_port_resv[i].proto == tuple->dst.protonum)) {
					if (((tuple->src.u.all >= lan_port_resv[i].min_port) && (tuple->src.u.all <= lan_port_resv[i].max_port)) ||
						((tuple->src.u.all <= lan_port_resv[i].min_port) && (tuple->src.u.all >= lan_port_resv[i].max_port))) {
							DEBUGP("tuple: %p, %u.%u.%u.%u %d <---> reserved ip: %u.%u.%u.%u %d-%d\n", tuple, NIPQUAD(tuple->src.ip), tuple->src.u.all, 
								   NIPQUAD(ignored_conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.ip),
								   lan_port_resv[i].min_port, lan_port_resv[i].max_port);
							DEBUGP("\n +++++ LAN port:%d, protcol:%d is conflict with current setting !! +++++\n\n", tuple->src.u.all, tuple->dst.protonum);
							return 1;
					}
				}
			}
	  ...
	  enum ip_nat_manip_type maniptype)
{
	...
	if (maniptype == IP_NAT_MANIP_SRC) {
		...
		iph->check = ip_nat_cheat_check(~saddr, target->src.ip, iph->check);
		u32_set(&iph->saddr, &target->src.ip);
	} else {
		__u32 daddr;
		u32_set(&daddr, &iph->daddr);
		iph->check = ip_nat_cheat_check(~daddr, target->dst.ip,	iph->check);
		u32_set(&iph->daddr, &target->dst.ip);
	}
#else
	if (maniptype == IP_NAT_MANIP_SRC) {
