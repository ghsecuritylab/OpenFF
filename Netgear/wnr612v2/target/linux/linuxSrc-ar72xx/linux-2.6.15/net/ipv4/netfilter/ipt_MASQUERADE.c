		  ...
		  void *userinfo)
{
	...
	strncpy(ct->nat.masq_ifname, out->name, IFNAMSIZ);
...
static inline int
device_cmp(struct ip_conntrack *i, void *ptr)
{
	int ret;
	struct net_device *dev = ptr;

	read_lock_bh(&masq_lock);
	ret = (strcmp(i->nat.masq_ifname, dev->name) == 0);
...
static inline int
inetdev_cmp(struct ip_conntrack *i, void *ptr)
{
	int ret = 0;
	struct in_ifaddr *ina = ptr;

	read_lock_bh(&masq_lock);
	if (!strcmp(i->nat.masq_ifname, ina->ifa_label) &&
...
{
	if (event == NETDEV_UP)
		ip_ct_iterate_cleanup(inetdev_cmp, ptr);
...
struct notifier_block masq_inet_notifier = {
