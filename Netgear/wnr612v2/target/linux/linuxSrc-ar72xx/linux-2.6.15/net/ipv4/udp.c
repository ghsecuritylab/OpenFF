 ...
 *
 * Version:	$Id: //depot/sw/releases/7.3_AP/linux/kernels/mips-linux-2.6.15/net/ipv4/udp.c#1 $
 *
 * Authors:	Ross Biro
...
#define SSDP_DELIVER_ALL	1

#if SSDP_DELIVER_ALL
#define SSDP_PORT	1900

/* 
  * Modified from `udp_v4_mcast_deliver` and `udp_v4_lookup_longway`, 
  * send SSDP-UDP-1900 packet to UPnP related sockets. 
  */
static int udp_v4_ssdp_deliver(struct sk_buff *skb)
{
	struct hlist_node *node;
	...
	sk_for_each(sk, node, &udp_hash[SSDP_PORT & (UDP_HTABLE_SIZE - 1)]) {
		struct inet_sock *inet = inet_sk(sk);
		if (inet->num == SSDP_PORT && !ipv6_only_sock(sk)) {
			if (sk_prev) {
				struct sk_buff *nskb = skb_clone(skb, GFP_ATOMIC);
				if (nskb) {
					if (udp_queue_rcv_skb(sk_prev, nskb) > 0)
						kfree_skb(nskb);
				}
			} 
			sk_prev = sk;
...
int udp_rcv(struct sk_buff *skb)
{
	...
	u32 saddr, daddr;
	u32_set(&saddr, &skb->nh.iph->saddr);
	u32_set(&daddr, &skb->nh.iph->daddr);
#else
	...
	if (uh->dest == __constant_htons(SSDP_PORT))
		return udp_v4_ssdp_deliver(skb);
#endif
