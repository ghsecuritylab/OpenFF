...
/*
 * MIPS CPU needs data address 32bit alignment for 4 octet size accessing.
 * Some PHY receives the packet with IP header 2 octet alignment, it makes
 * the original 'ip_fast_csum' == NOT FAST ==.
 */
static inline unsigned short ip_fast_csum(unsigned char *iph, unsigned int ihl)
{
	unsigned int csum;
	__u16 *packet = (__u16 *) iph;
	__u16 *stop = packet + (ihl << 1);
