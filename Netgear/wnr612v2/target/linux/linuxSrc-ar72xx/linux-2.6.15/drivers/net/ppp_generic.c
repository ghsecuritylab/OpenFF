			 ...
			 size_t count, loff_t *ppos)
{
	...
	if (pf == 0)
		return -ENXIO;
	ret = -ENOMEM;
	skb = alloc_skb(count + pf->hdrlen+2, GFP_KERNEL);
	if (skb == 0)
		goto out;
	skb_reserve(skb, pf->hdrlen+2);
	ret = -EFAULT;
...
/* 
 * NetGear Spec 5.5 PPP dial on demand (p65): 
 ...
 * update detection. 
 */
static int dod_filter(char *data)
{
	struct iphdr *iph = (struct iphdr *)(data + 2);	/* skip two ppp proto octets */
	...
	if (iph->protocol == IPPROTO_UDP) {
		...
		if (udph->dest == __constant_htons(53)) {
			if (ntohs(udph->len) < (sizeof(struct udphdr) + 13 + 5)) {
				DOD_DEBUG("This DNS query packet seems too short.\n");
				return 1;
			}

			/* 
 			* DNS packet has 12 fixed octet header, then the domain name and etc ...
 			* the domain is formatted as bellow:
 			* 		gemini.tuc.noao.edu --> 6gemini3tuc4noao3edu0
 			* */
...
#ifdef DNI_DOD
		if (no_filter == 0) {
			DOD_DEBUG("PPP is still in idle.\n");
			goto drop;
		}
#endif
