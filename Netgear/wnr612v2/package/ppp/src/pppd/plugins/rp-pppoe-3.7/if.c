...
"$Id: if.c,v 1.2 2006/03/20 11:07:17 alva_zhang Exp $";
...
openInterface(char const *ifname, UINT16_t type, unsigned char *hwaddr)
{
   ...
   syslog(LOG_ERR, "===>PPP Session: Opening a BPF on an interface for all PPPoE traffic......!\n");
	...
	case EACCES:		/* permission denied */
		...
		syslog(LOG_ERR, "=====>Open BPF Interf: Failed, Cannot open %.32s -- pppoe must be run as root.\n", bpfName);
		rp_fatal(buffer);
	    }
	    break;
	case EBUSY:
	case ENOENT:		/* no such file */
	    if (i == 0) {
		syslog(LOG_ERR, "=====>Open BPF Interf: Failed, No /dev/bpf* devices (check your kernel configuration for BPF support)\n");
		rp_fatal("No /dev/bpf* devices (check your kernel configuration for BPF support)");
	    } else {
	        syslog(LOG_ERR, "=====>Open BPF Interf: Failed, All /dev/bpf* devices are in use!\n");
		rp_fatal("All /dev/bpf* devices are in use");
	    }
	    break;
	}
	syslog(LOG_ERR, "=====>Open BPF Interf: Failed, pppoe-discovery: %.256s\n",bpfName);
	fatalSys(bpfName);
    }

    if ((sock = socket(AF_LOCAL, SOCK_DGRAM, 0)) < 0) {
	syslog(LOG_ERR, "=====>Open BPF Interf: Failed, open socket\n");	
	fatalSys("socket");
    }
	...
	syslog(LOG_ERR, "=====>Open BPF Interf: ioctl(SIOCGIFFLAGS) Failed!\n");		
	fatalSys("ioctl(SIOCGIFFLAGS)");
    }
    if ((ifr.ifr_flags & IFF_UP) == 0) {
	char buffer[256];
	sprintf(buffer, "Interface %.16s is not up\n", ifname);
	syslog(LOG_ERR, "=====>Open BPF Interf: Failed, Interface %.16s is not up!\n", ifname);	
	rp_fatal(buffer);
    }

    /* Fill in hardware address and initialize the packet filter rules */
    if (hwaddr == NULL) {
	syslog(LOG_ERR, "=====>Open BPF Interf: Failed, no hwaddr arg!\n");	
	...
	syslog(LOG_ERR, "=====>Open BPF Interf: ioctl(SIOCGIFMTU) Failed!\n");		
	...
	syslog(LOG_ERR, "=====>Open BPF Interf: Interface %.16s has MTU of %d -- should be %d.",ifname, ifr.ifr_mtu, ETH_DATA_LEN);
	printErr(buffer);
    }
#endif
    ...
    if (close(sock) < 0) {
	syslog(LOG_ERR, "=====>Open BPF Interf: Failed,socket closed!\n");	
	fatalSys("close");
    }

    /* Check the BPF version number */
    if (ioctl(fd, BIOCVERSION, &bpf_ver) < 0) {
	syslog(LOG_ERR, "=====>Open BPF Interf: ioctl(BIOCVERSION) Failed!\n");	
	...
	syslog(LOG_ERR, "=====>Open BPF Interf: Failed, %s!\n",buffer);	
	rp_fatal(buffer);
    }

    /* allocate a receive packet buffer */
    if (ioctl(fd, BIOCGBLEN, &bpfLength) < 0) {
	syslog(LOG_ERR, "=====>Open BPF Interf: ioctl(BIOCGBLEN) Failed!\n");	
	fatalSys("ioctl(BIOCGBLEN)");
    }
    if (!(bpfBuffer = (unsigned char *) malloc(bpfLength))) {
	syslog(LOG_ERR, "=====>Open BPF Interf: malloc Failed!\n");	
	rp_fatal("malloc");
    }
	...
	syslog(LOG_ERR, "=====>Open BPF Interf: ioctl(BIOCIMMEDIATE) Failed!\n");	
	...
	sprintf(buffer, "ioctl(BIOCSETIF) can't select interface %.16s",
		ifname);
	syslog(LOG_ERR, "=====>Open BPF Interf: ioctl(BIOCSETIF) can't select interface %.16s!\n", ifname);
	rp_fatal(buffer);
    }
    ...
    syslog(LOG_INFO, "Interface=%.16s HWaddr=%02X:%02X:%02X:%02X:%02X:%02X Device=%.32s Buffer size=%d",
	   ...
	   bpfName, bpfLength);
    syslog(LOG_ERR, "===>PPP Session: Succeeded to Open a BPF on an interface for all PPPoE traffic!\n");
    return fd;
}
