...
*
* Copyright (C) 2000-2005 by Roaring Penguin Software Inc.
*
...
"$Id: pppoe.c,v 1.3 2006/03/20 11:07:48 alva_zhang Exp $";
...
session(PPPoEConnection *conn)
{
    ...
    syslog(LOG_ERR, "===>PPP Session: Initializes the PPP state machine!\n");
    initPPP();

#ifdef USE_BPF
    /* check for buffered session data */
     //syslog(LOG_ERR, "===>PPP Session: check for buffered session data......\n");
	...
	if (r < 0) {
	    syslog(LOG_ERR, "===>PPP Session:Failed to select (session)!\n");	
	    fatalSys("select (session)");
	}
	if (r == 0) { /* Inactivity timeout */
	    syslog(LOG_ERR, "===>PPP Session:Failed,Inactivity timeout... something wicked happened on session %d", (int) ntohs(conn->session));
   ...
   syslog(LOG_ERR, "===>PPP Session:Succeeded to session !\n");
}
...
usage(char const *argv0)
{
    ...
    fprintf(stderr, "PPPoE Version %s, Copyright (C) 2001-2005 Roaring Penguin Software Inc.\n", VERSION);
...
main(int argc, char *argv[])
{
    ...
    while((opt = getopt(argc, argv, "I:VAT:D:hS:C:Usm:np:e:kdf:F:t:i:")) != -1) {
	switch(opt) {
	case 't':
	...
	case 'h':
	    usage(argv[0]);
	    break;
#if 1 //tony: patch for romeDriver
	case 'i':
		rtl865x_dsid = (int) strtol(optarg, NULL, 10);
		printf("pppoe dial session id: %u\n",rtl865x_dsid);
		break;
#endif
	...
	for (n=0; n < optFloodDiscovery; n++) {
	    ...
	    discovery(&conn);
	    //syslog(LOG_ERR, "==>PPPD: main line 619--!\n");
    ...
    discovery(&conn);
       //syslog(LOG_ERR, "==>PPPD: main line 626--!\n");
    if (optSkipSession) {
    ...
    syslog(LOG_ERR, "==>PPPD: main line 644--!\n");
    session(&conn);
    return 0;
}
...
asyncReadFromEth(PPPoEConnection *conn, int sock, int clampMss)
{
    ...
    /* Check length */
    if (ntohs(packet.length) + HDR_SIZE > len) {
	syslog(LOG_ERR, "=====>asyncReadFromEth: Failed, Bogus PPPoE length field (%u)",
	       (unsigned int) ntohs(packet.length));
	return;
    }
    if (conn->debugFile) {
	...
	syslog(LOG_ERR, "=====>asyncReadFromEth: Failed, Unexpected packet code %d", (int) packet.code);
	return;
    }
    if (packet.ver != 1) {
	syslog(LOG_ERR, "=====>asyncReadFromEth: Failed, Unexpected packet version %d", (int) packet.ver);
	return;
    }
    if (packet.type != 1) {
	syslog(LOG_ERR, "=====>asyncReadFromEth: Failed, Unexpected packet type %d", (int) packet.type);
	return;
    }
    if (memcmp(packet.ethHdr.h_dest, conn->myEth, ETH_ALEN)) {
	return;
	...
	syslog(LOG_ERR, "=====>asyncReadFromEth: Failed, Bogus length field in session packet %d (%d)",
	       (int) plen, (int) len);
	return;
    }
    ...
    *ptr++ = FRAME_FLAG;
	...
	syslog(LOG_ERR,"=====>asyncReadFromEth: Failed, asyncReadFromEth: write!\n");
	fatalSys("asyncReadFromEth: write");
    }
   // syslog(LOG_ERR, "===>PPP Session: Succeeded to Reads a packet from the Ethernet interface and sends it to async PPP!\n");
}
...
syncReadFromEth(PPPoEConnection *conn, int sock, int clampMss)
{
    ...
    /* Check length */
    if (ntohs(packet.length) + HDR_SIZE > len) {
	syslog(LOG_ERR, "=====>syncReadFromEth: Failed, Bogus PPPoE length field (%u)\n",
	       (unsigned int) ntohs(packet.length));
	return;
    }
    if (conn->debugFile) {
    ...
    /* Sanity check */
    if (packet.code != CODE_SESS) {
	syslog(LOG_ERR, "=====>syncReadFromEth: Failed, Unexpected packet code %d\n", (int) packet.code);
	return;
    }
    if (packet.ver != 1) {
	syslog(LOG_ERR, "=====>syncReadFromEth: Failed, Unexpected packet version %\nd", (int) packet.ver);
	return;
    }
    if (packet.type != 1) {
	syslog(LOG_ERR, "=====>syncReadFromEth: Failed, Unexpected packet type %d\n", (int) packet.type);
	return;
    }
    if (memcmp(packet.ethHdr.h_dest, conn->myEth, ETH_ALEN)) {
	/* Not for us -- must be another session.  This is not an error,
    ...
    plen = ntohs(packet.length);
    if (plen + HDR_SIZE > len) {
	syslog(LOG_ERR, "=====>syncReadFromEth: Failed, Bogus length field in session packet %d (%d)\n",
	       (int) plen, (int) len);
	return;
    }
    ...
    if (writev(1, vec, 2) < 0) {
	syslog(LOG_ERR, "=====>syncReadFromEth: Failed, syncReadFromEth: write\n");
	fatalSys("=====>syncReadFromEth: Failed, syncReadFromEth: write");
    }
  //syslog(LOG_ERR, "===>PPP Session: Succeeded to Reads a packet from the Ethernet interface and sends it to sync PPP!\n");
}
