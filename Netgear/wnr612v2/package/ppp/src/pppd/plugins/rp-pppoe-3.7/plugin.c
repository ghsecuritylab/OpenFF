...
"$Id: plugin.c,v 1.2 2006/03/20 11:07:28 alva_zhang Exp $";
...
extern struct server_list  entry_mac[MAX_ENTRY];
void
init_entry_table(void)
{
        int i;

        printf("init entry tables ....\n");
        for(i=0; i<MAX_ENTRY; i++){
            memset(entry_mac[i].mac, 0, ETH_ALEN);
            entry_mac[i].count=-1;
            entry_mac[i].flag=0;
        }
}
...
PPPOEConnectDevice(void)
{
    ...
    syslog(LOG_ERR, "==>PPPD: PPPOEConnectDevice--!\n");
    ...
    if (sscanf(existingSession, "%d:%x:%x:%x:%x:%x:%x",
           ...
           &mac[3], &mac[4], &mac[5]) != 7) {
      syslog(LOG_ERR, "==>PPPD:Illegal value for rp_pppoe_sess option!\n");
    ...
    if (conn->discoveryState != STATE_SESSION) {
      syslog(LOG_ERR, "==>PPPD:  Unable to complete PPPoE Discovery!\n");
    ...
    if (conn->sessionSocket < 0) {
      syslog(LOG_ERR, "===>PPP Session: Failed to create PPPoE socket!\n");
      error("Failed to create PPPoE socket: %m");
      return -1;
    }
    syslog(LOG_ERR, "===>PPP Session: Succeeded to create PPPoE socket!\n");
    ...
    if (connect(conn->sessionSocket, (struct sockaddr *) &sp,
      ...
      syslog(LOG_ERR, "===>PPP Session: Failed to connect PPPoE socket: %d \n", errno);
      error("Failed to connect PPPoE socket: %d %m", errno);
    return -1;
    }
    syslog(LOG_ERR, "===>PPP Session: Succeeded to connect PPPoE socket\n");
        ...
        int accomp)
{
    int sock;
    struct ifreq ifr;
    syslog(LOG_ERR, "===>PPP Session:  send PPPOE configure......\n");
    ...
    if (sock < 0) {
      syslog(LOG_ERR, "=====>PPP Config: Failed to create  IP socket!\n");
    ...
    if (ioctl(sock, SIOCSIFMTU, &ifr) < 0) {
      syslog(LOG_ERR, "=====>PPP Config: Failed to set interface MTU to %d \n", mtu);
      warn("ioctl(SIOCSIFMTU): %m");
      return;
    }
    (void) close (sock);
    syslog(LOG_ERR, "===>PPP Session:  Succeeded to send PPPOE configure!\n");
...
// add by kane.chen
// send PADT to close PPPoE connection 
static void PPPoECloseConnection(const char *message)
{
	sendPADT(conn, message);
}

static void PPPoESetCleanFlag(int flag)
{
	clean_session_id = flag;
}
#endif
...
struct channel pppoe_channel = {
    ...
    .cleanup = NULL
#if (NETGEAR_ROUTER_SPEC)
    ,.close_connection = &PPPoECloseConnection
    ,.set_clean_flag = &PPPoESetCleanFlag
#endif
};
