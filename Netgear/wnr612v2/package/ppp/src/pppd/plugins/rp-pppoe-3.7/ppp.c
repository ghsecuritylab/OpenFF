...
"$Id: ppp.c,v 1.2 2006/03/20 11:07:38 alva_zhang Exp $";
...
syncReadFromPPP(PPPoEConnection *conn, PPPoEPacket *packet)
{
	...
	if (errno == EAGAIN) {syslog(LOG_ERR, "=====>syncReadFromPPP: Failed, Linux select bug hit! This message is harmless, but please ask the Linux kernel developers to fix it.");
	    rp_fatal("Linux select bug hit!  This message is harmless, but please ask the Linux kernel developers to fix it.");
	}
	syslog(LOG_ERR, "=====>syncReadFromPPP: Failed, read (syncReadFromPPP)");
	fatalSys("read (syncReadFromPPP)");
    }
    if (r == 0) {
	syslog(LOG_ERR, "=====>syncReadFromPPP: Failed, end-of-file in syncReadFromPPP");
	sendPADT(conn, "RP-PPPoE: EOF in syncReadFromPPP");
	exit(0);
    }

    if (r < 2) {
	syslog(LOG_ERR, "=====>syncReadFromPPP: Failed, too few characters read from PPP (syncReadFromPPP)\n");
...
asyncReadFromPPP(PPPoEConnection *conn, PPPoEPacket *packet)
{
    ...
    if (r < 0) {
	syslog(LOG_ERR, "=====>asyncReadFromPPP: Failed to read (asyncReadFromPPP)!\n");
	fatalSys("read (asyncReadFromPPP)");
    }

    if (r == 0) {
		
	syslog(LOG_ERR, "=====>asyncReadFromPPP: Failed, end-of-file in asyncReadFromPPP\n");
	...
	while(r && PPPState == STATE_BUILDING_PACKET) {
	    ...
	    case FRAME_FLAG:
		if (PPPPacketSize < 2) {
		    syslog(LOG_ERR, "=====>asyncReadFromPPP: Failed, Packet too short from PPP (asyncReadFromPPP)");
		...
		if (PPPPacketSize >= ETH_DATA_LEN - 4) {
		    syslog(LOG_ERR, "=====>asyncReadFromPPP: Failed, Packet too big!  Check MTU on PPP interface");
		    PPPPacketSize = 0;
		    PPPXorValue = 0;
		    PPPState = STATE_WAITFOR_FRAME_ADDR;
		} else {
