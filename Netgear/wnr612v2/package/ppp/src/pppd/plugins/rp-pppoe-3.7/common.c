...
"$Id: common.c,v 1.1 2005/11/30 07:47:37 tony Exp $";
...
unsigned char AllZeroMac[6]={0, 0, 0, 0, 0, 0};
#endif
void
sendPADT(PPPoEConnection *conn, char const *msg)
{
	...
	if (fp)
	{
		...
		{
			fseek(fp, BLOCK_SIZE + BUFFER_OFFSET, SEEK_SET);
...
#if (NETGEAR_ROUTER_SPEC)
    /* 
      * if we don't have any peer infomation, it means pppd is started first time.
      * try to send PADT with last peer mac address.
      */
	if (fp)
    {
		if (!memcmp(AllZeroMac,conn->peerEth, ETH_ALEN))
		{
			clean_session_id = 1;
			if (line)
				p = strchr(line, ':');
			...
			{
				p++;
				memcpy(packet.ethHdr.h_dest, p, ETH_ALEN);
			}
			else
				memset(packet.ethHdr.h_dest, 0xff, ETH_ALEN); /* it should not happen */
		}
		else
