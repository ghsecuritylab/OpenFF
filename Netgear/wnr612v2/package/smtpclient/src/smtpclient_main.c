...
char project_name[BUFSIZ] = "WNR612v2";
...
void usage(void)
{
    ...
    fprintf(stderr, "  -n,                    setting Target host name, default NULL\n");
...
int main(int argc, char **argv)
{
     ...
     *  Parse options
     */
    while ((c = getopt_long(argc, argv, ":n:s:f:r:e:c:S:p:MLvVhU:P:", options, NULL)) != EOF) {
        switch (c) {
            case 's':
                subject = optarg;
                break;
            ...
            case 'n':
                if(strlen(optarg)> BUFSIZ) {
                    printf("The projects name length is excessively long.\n");
                    break;
                }
