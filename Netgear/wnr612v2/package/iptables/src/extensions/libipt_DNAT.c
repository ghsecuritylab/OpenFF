...
" --dnat-type [forwarding | triggering | upnp | dmz ]\n\n",
IPTABLES_VERSION);
}
      ...
      struct ipt_entry_target **target)
{
	...
	case '1':
		...
		*flags = 1;
		return 1;
	case '2':
		if (!strcasecmp(optarg, "forwarding"))
			info->mr.range[0].flags |= IPS_FORWARDING;
		else if (!strcasecmp(optarg, "upnp"))
			info->mr.range[0].flags |= IPS_UPNP;
		else if (!strcasecmp(optarg, "dmz"))
			info->mr.range[0].flags |= IPS_DMZ;
		else
			exit_error(PARAMETER_PROBLEM,
				   "unknown dnat_type `%s' specified", optarg);
		return 1;
