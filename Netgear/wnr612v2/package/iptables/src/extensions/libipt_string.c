...
"--get-only [0|1]			Check GET string in a packet\n",
"--trusted-ip string	Setting Trusted IP address\n",
IPTABLES_VERSION);
}
...
static struct option opts[] = {
	...
	{ .name = "get-only", .has_arg = 1, .flag =0, .val = '3' },
	{ .name = "trusted-ip", .has_arg = 1, .flag =0, .val = '4' },
	{ .name = 0 }
};
      ...
      struct ipt_entry_match **match)
{
	...
	case '3':
		stringinfo->is_get=atoi(optarg);
		*flags = 1;
		break;
	case '4'://for Trusted IP address
		stringinfo->TrustedIPaddr=inet_addr(optarg);
		break;
	default:
		return 0;
	}
...
final_check(unsigned int flags)
{
	if (!flags)
		exit_error(PARAMETER_PROBLEM,
			   "STRING match: You must specify `--string' or `--hex-string' or `--get-only' ");
}
      ...
      int numeric)
{
	...
	if (info->is_get ==1)
		printf("--get-only %d", info->is_get);

	if(info->TrustedIPaddr != 0){
		printf("Trusted IP %s ", intoa(info->TrustedIPaddr));
	}
}
...
save(const struct ipt_ip *ip, const struct ipt_entry_match *match)
{
	...
	if (info->is_get == 1)
		printf("--get-only %d", info->is_get);
	if(info->TrustedIPaddr != 0){
		printf("Trusted IP 0x%x ", info->TrustedIPaddr);
	}
}
