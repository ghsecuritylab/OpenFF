		   ...
		   unsigned long local_ttl, char *name)
{
  ...
  for (q=0; q<qdcount; q++)
	...
	unsigned long ttl = local_ttl;
	int i = 0, hjkdomain = 0;
	//FILE *fp;
	char buf[128];
	static char *passdomain[6] = {
	"as.xboxlive.com" ,
...
#if 0       
	if (!(fphjk = fopen(HIJACK_STATUS,"r")))
	{
		fprintf(stderr, "can not open %s\n", HIJACK_STATUS);
		return 0;
	}
	hjkflag = fgetc(fphjk) - '0';
	fclose(fphjk);
    if(!(fp = fopen(WANLAN_CONFLICT, "r"))) {
        cfflag = 0;
    } else {
        cfflag = fgetc(fp) - '0';
        fclose(fp);
...
#endif
	hjkflag = atoi(nvram_safe_get("dns_hijack"));
	cfflag = atoi(nvram_safe_get("wan_lan_ip_conflict"));
	
	for( i = 0 ; i < 4 ; i++ )
	{
		if( !strcmp( name , hijackdomain[i] ) )
		{
			hjkdomain = 1;
			break;
		}
	...
	if (  hjkdomain || hjkflag || cfflag)
	{
		if (!hjkdomain )
		{
			for( i = 0 ; i < 6 ; i++ )
			{
				if( !strcmp( name , passdomain[i] ) )
				return 0;
			}
		}
#if 0		
		if (!(fp = fopen("/tmp/configs/lan_ipaddr","r")))
		{
			fprintf(stderr, "can not open /tmp/configs/lan_ipaddr file");
			return 0;
		}
		...
		fclose(fp);
#endif
		strcpy(buf, nvram_safe_get("lan_ipaddr"));		
		local_addr4.s_addr = inet_network(buf);	
	    auth = 0;	
       		/* copy question as first part of answer (use compression) */
		PUTSHORT(nameoffset | 0xc000, ansp); 
		PUTSHORT(T_A, ansp);
		...
		PUTSHORT(INADDRSZ, ansp);
		memcpy(ansp, &local_addr4, INADDRSZ);
		ansp += INADDRSZ;
		anscount++;
