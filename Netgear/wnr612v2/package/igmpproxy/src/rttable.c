...
static inline int
wireless_leave_group(unsigned long src, unsigned long group)
{
	char line[64];
	if (mc_ctrl_packet(group))
		return 0;
	sprintf(line, "echo \"d %u.%u.%u.%u %u.%u.%u.%u \" > /proc/wl_mcast", 
		NIPQUAD(src), NIPQUAD(group));
//	DPRINTF("%s\n",line);
	system(line);
	return 0;
...
static inline int
remove_group(unsigned long group)
{
	char line[64];
	if (mc_ctrl_packet(group))
		return 0;
	sprintf(line, "echo \"remove %u.%u.%u.%u \" > /proc/mcast_entry", NIPQUAD(group));
//	DPRINTF("%s\n",line);	
	system(line);
	return 0;
}
...
void check_mbr_timeout(void)
{
	...
	timer_setTimer( INTERVAL_CHECK_MEMBER, check_mbr_timeout, NULL);
}
...
void setRouteLastMemberMode(uint32 group) {
    ...
    if(croute!=NULL) {
		...
		wireless_leave_group(src, group);		
#endif
        }
...
int removeRoute(struct RouteTable*  croute) {
    ...
    if (croute->listeners)
    {
	  ...
	  while (nextlistener)
	  {
		...
		wireless_leave_group(listener->srcAddr, croute->group);
#endif
	  	free(listener);
