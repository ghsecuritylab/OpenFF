...
void initCommonConfig() {
    ...
    commonConfig.startupQueryInterval = INTERVAL_QUERY;//(unsigned int)(INTERVAL_QUERY / 4);
...
#if IGMP_QUERY_PATCH
	commonConfig.query_count = IGMP_QUERY_DEFAULT_COUNT;
#endif
}
...
int loadConfig(char *configFile) {
    ...
    if(!openConfigFile(configFile)) {
        igmp_syslog(LOG_ERR, 0, "Unable to open configfile from %s", configFile);
    }

    // Get first token...
    token = nextConfigToken();
    if(token == NULL) {
        igmp_syslog(LOG_ERR, 0, "Config file was empty.");
    }
    ...
    while ( token != NULL ) {
        ...
        if(strcmp("phyint", token)==0) {
            ...
            IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Config: Got a phyint token.");
            tmpPtr = parsePhyintToken();
            if(tmpPtr == NULL) {
                // Unparsable token... Exit...
                closeConfigFile();
                igmp_syslog(LOG_WARNING, 0, "Unknown token '%s' in configfile", token);
                return 0;
            } else {

                IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "IF name : %s", tmpPtr->name);
                IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Next ptr : %x", tmpPtr->next);
                IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Ratelimit : %d", tmpPtr->ratelimit);
                IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Threshold : %d", tmpPtr->threshold);
                IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "State : %d", tmpPtr->state);
                IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Allowednet ptr : %x", tmpPtr->allowednets);
            ...
            IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Config: Quick leave mode enabled.");
            ...
            // Got a mode token...
	    token = nextConfigToken();
            IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Config: set mode to '%s'.", token);
...
#endif
#if IGMP_QUERY_PATCH
		}else if (strcmp("query_count", token)==0) {
			token = nextConfigToken();
			commonConfig.query_count = atoi( token );
			// to limit the query count is between 1 to 9
			if (commonConfig.query_count > 9)
				commonConfig.query_count = 9;
			else if (commonConfig.query_count < 1)
				commonConfig.query_count = 1;
			token = nextConfigToken();
			continue;
#endif
        } else {
        ...
        if ( Dp->InAdr.s_addr && ! (Dp->Flags & IFF_LOOPBACK) ) {
            ...
            for( confPtr = vifconf; confPtr; confPtr = confPtr->next) {
                ...
                if(strcmp(Dp->Name, confPtr->name)==0) {
                    ...
                    IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Found config for %s", Dp->Name);
...
struct vifconfig *parsePhyintToken() {
    ...
    IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Config: IF: Config for interface %s.", token);

    // Allocate memory for configuration...
    tmpPtr = (struct vifconfig*)malloc(sizeof(struct vifconfig));
    if(tmpPtr == NULL) {
        igmp_syslog(LOG_ERR, 0, "Out of memory.");
    }
    ...
    while(token != NULL) {
        if(strcmp("altnet", token)==0) {
            ...
            token = nextConfigToken();
            IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Config: IF: Got altnet token %s.",token);

            *anetPtr = parseSubnetAddress(token);
            if(*anetPtr == NULL) {
                parseError = 1;
                igmp_syslog(LOG_WARNING, 0, "Unable to parse subnet address.");
                break;
            } else {
            ...
            // Upstream
            IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Config: IF: Got upstream token.");
            tmpPtr->state = IF_STATE_UPSTREAM;
        }
        else if(strcmp("downstream", token)==0) {
            // Downstream
            IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Config: IF: Got downstream token.");
            tmpPtr->state = IF_STATE_DOWNSTREAM;
        }
        else if(strcmp("disabled", token)==0) {
            // Disabled
            IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Config: IF: Got disabled token.");
            tmpPtr->state = IF_STATE_DISABLED;
        }
        else if(strcmp("ratelimit", token)==0) {
            // Ratelimit
            token = nextConfigToken();
            IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Config: IF: Got ratelimit token '%s'.", token);
            tmpPtr->ratelimit = atoi( token );
            if(tmpPtr->ratelimit < 0) {
                igmp_syslog(LOG_WARNING, 0, "Ratelimit must be 0 or more.");
                parseError = 1;
                break;
            }
        }
            ...
            // Threshold
            token = nextConfigToken();
            IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Config: IF: Got threshold token '%s'.", token);
            tmpPtr->threshold = atoi( token );
            if(tmpPtr->threshold <= 0 || tmpPtr->threshold > 255) {
                igmp_syslog(LOG_WARNING, 0, "Threshold must be between 1 and 255.");
                parseError = 1;
                break;
            }
        }
...
struct SubnetList *parseSubnetAddress(char *addrstr) {
    ...
    if(tmpStr != NULL) {
        ...
        if(bitcnt <= 0 || bitcnt > 32) {
            igmp_syslog(LOG_WARNING, 0, "The bits part of the address is invalid : %d.",tmpStr);
            return NULL;
        }
    ...
    if(addr == -1 || addr == 0) {
        igmp_syslog(LOG_WARNING, 0, "Unable to parse address token '%s'.", addrstr);
        return NULL;
    }
    ...
    IF_DEBUG igmp_syslog(LOG_DEBUG, 0, "Config: IF: Altnet: Parsed altnet to %s.",
