...
BOOL SendKeepAlive(PHGlobal *phglobal, int opCode)
{
	...
	pData = p2;
	Change_Endianness(&(pData->lChatID));
	Change_Endianness(&(pData->lID));
	Change_Endianness(&(pData->lOpCode));
	Change_Endianness(&(pData->lSum));
	Change_Endianness(&(pData->lReserved));
...
{
	char temp[100];
	DATA_KEEPALIVE_EXT rdata, *prData;
	DATA_KEEPALIVE data, *pData;
	blf_ctx blf;
	char p1[KEEPALIVE_PACKET_LEN],p2[KEEPALIVE_PACKET_LEN];
	...
	pData = &(prData->keepalive);
	Change_Endianness(&(pData->lChatID));
	Change_Endianness(&(pData->lID));
	Change_Endianness(&(pData->lOpCode));
	Change_Endianness(&(pData->lSum));
	Change_Endianness(&(pData->lReserved));
	Change_Endianness(&(prData->ip));
...
int ExecuteUpdate(PHGlobal *phglobal)
{
		...
		if (pos0)
		{
			...
			if (pos1)
			{
				*pos1 = '\0';
				...
				config_set("oray_serv_type", (pos0 + 1));
			}
		}
	}
	...
	for (i=0;i<totaldomains-1;i++)
    {
		...
		if (phglobal->cbOnDomainRegistered)
		{ 
			phglobal->cbOnDomainRegistered(domains[i]);
  		strcat(domain_list, domains[i]);
			strcat(domain_list, " ");
  	}
  }
  strcpy(hostname, domain_list);
	config_set("oray_host_list", domain_list);
	...
	for (;;)
	{
		...
		{
			buflen += MAX_PATH;
			xmldata = (char *) realloc(xmldata, buflen);
			memset((xmldata + buflen) - MAX_PATH, 0, MAX_PATH);
		}
		strcat(xmldata, buffer);
	}
	LOG(1) ("userinfo: \r\n%s\r\n", xmldata);
...
int phddns_step(PHGlobal *phglobal)
{
	...
	if (phglobal->bNeed_connect)
	{
		...
		/* DNI add : AuthFailed, maybe username or password error, never do update. */
                if(ret == errorAuthFailed) exit(0);
                /* end of DNI add */
