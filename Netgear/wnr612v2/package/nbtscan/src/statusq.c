...
#ifdef NETGEAR 
	status = sendto(sock, (char*)&request, sizeof(request) - 2, 0,
...
struct nb_host_info* parse_response(char* buff, int buffsize) {
	...
	//offset -=(response_header->number_of_names)* 2;
	...
	/*printf("###############struct size %d num size %d number_of_names %d %02X:%02X:%02X:%02X:%02X:%02X;%02X:%02X:%02X:%02X:%02X:%02X\n",
			sizeof(struct nbname),
			sizeof(response_header->number_of_names),
			response_header->number_of_names,
			*(char *)(buff+offset), *(char *)(buff+offset+1),
			*(char *)(buff+offset+2), *(char *)(buff+offset+3),
			*(char *)(buff+offset+4), *(char *)(buff+offset+5),
			*(char *)(buff+offset+6), *(char *)(buff+offset+7),
			*(char *)(buff+offset+8), *(char *)(buff+offset+9),
			*(char *)(buff+offset+10), *(char *)(buff+offset+11));
