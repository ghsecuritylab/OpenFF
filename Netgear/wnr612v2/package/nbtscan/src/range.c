...
/* is_range1 checks if supplied string is an IP address range in
 * form xxx.xxx.xxx.xxx/xxx.xxx.xxx.xxx (as in 192.168.1.2/225.255.255.0) and fills
 * range structure with start and end ip addresses of the interval.
 * Returns 1 on success, 0 on failure */

int is_range3(char* string, struct ip_range* range) {
	char* separator;
	unsigned long mask;
	...
	if(separator=(char*)strchr(string,'/')) {
		if (strlen(separator)<6) {
			free(ip);
			return 0;
		}
		separator++;
		if((mask = inet_addr(separator)) == INADDR_NONE){
			free(ip);
			return 0;
		}
		strcpy(ip, string);
		...
		range->end_ip = ntohl((start_ip) | (~mask));
		range->start_ip=ntohl((start_ip) & mask);
