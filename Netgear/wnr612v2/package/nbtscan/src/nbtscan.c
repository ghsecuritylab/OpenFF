...
int set_range(char* range_str, struct ip_range* range_struct) {
  ...
  if(is_range3(range_str, range_struct)) return 1;
...
int print_hostinfo(struct in_addr addr, struct nb_host_info* hostinfo, char* sf, FILE *fp, int tofile) {
  int i;
  unsigned char service; /* 16th byte of NetBIOS name */
  ...
  if(tofile){
	fprintf(fp,"%s %s\n",inet_ntoa(addr) ,comp_name);
/*
if(!((hostinfo->footer)&&(hostinfo->footer->adapter_address[0] 
...
int main(int argc, char *argv[]) {
  int tofile=1, timeout=10000, verbose=0, use137=0, ch, dump=0, bandwidth=0, send_ok=0, hr=0, etc_hosts=0, lmhosts=0;
  extern char *optarg;
  extern int optind;
  char* target_string;
  char* sf=NULL;
  ...
  FILE *dev_fp;
  FILE *iplist_fp, *ip_mac_fp, *ip_name_fp;

  unlink("/tmp/netscan/attach_device");
  unlink("/tmp/ip_mac_list");
  unlink("/tmp/ip_name_list");
  unlink("/tmp/iplist-nbt");
  ip_mac_fp = fopen("/tmp/ip_mac_list","w+");
  if(ip_mac_fp == NULL)
		exit(-1);
  ip_name_fp = fopen("/tmp/ip_name_list","w+");
  if(ip_name_fp == NULL)
  {
		fclose(ip_mac_fp);
		exit(-1);
  }
  dev_fp = fopen("/tmp/netscan/attach_device","w+");
  if(dev_fp == NULL)
  { 
        fclose(ip_mac_fp);
		fclose(ip_name_fp);
  ...
  iplist_fp = fopen("/tmp/iplist-nbt","w+");
  if(iplist_fp == NULL){
		fclose(ip_mac_fp);
        fclose(ip_name_fp);
        fclose(dev_fp);
  ...
  while ((ch = getopt(argc, argv, "pvrdelqhm:s:t:b:f:")) != -1)
    switch (ch) {
    case 'p':
	tofile = 0;
	break;	
  ...
  for(i=0; i <= retransmits; i++) {
    ...
    while ( more_to_send) {
      if(1) {
	if ( (size = recvfrom(arp_sock, buff, BUFFSIZE, 0,
			      &SA, sizeof(SA))) >= sizeof(struct arp_packet) ) {

	//hostinfo = (struct nb_host_info*)parse_response(buff, size);
	arp_pkt = (struct arp_packet *)buff;
	
	if((ntohs(arp_pkt->type) == 2) &&(MAC_CMP(g_local_mac, arp_pkt) == 0)){
		char src_hw_addr[18];
    	char src_ip[16];
		struct sockaddr_in ina;

           sprintf(src_hw_addr, "%02X:%02X:%02X:%02X:%02X:%02X",
            arp_pkt->sndr_hw_addr[0],
            arp_pkt->sndr_hw_addr[1],
            arp_pkt->sndr_hw_addr[2],
            arp_pkt->sndr_hw_addr[3],
            arp_pkt->sndr_hw_addr[4],
            arp_pkt->sndr_hw_addr[5]);
           sprintf(src_ip,"%d.%d.%d.%d",
            arp_pkt->sndr_ip_addr[0],
            arp_pkt->sndr_ip_addr[1],
            arp_pkt->sndr_ip_addr[2],
            arp_pkt->sndr_ip_addr[3]);

         ina.sin_addr.s_addr = inet_addr(src_ip);
		
	  /* If this packet isn't a duplicate */
	 if(insert(scanned, ntohl(ina.sin_addr.s_addr))) {

		fprintf(ip_mac_fp,"%s %s\n", src_ip, src_hw_addr);
		fprintf(iplist_fp,"%s\n", src_ip);
	 }
	...
	if(next_address(&range, prev_in_addr, next_in_addr) ) {
	  if(!in_list(scanned, ntohl(next_in_addr->s_addr))) 
		send_arp_req(arp_sock, *next_in_addr, &SA);
  ...
  for(i=0; i <= retransmits; i++) {
	...
	if(insert(scanned, ntohl(dest_sockaddr.sin_addr.s_addr))) {
	  ...
	  if (verbose) 
	    ...
	    print_hostinfo(dest_sockaddr.sin_addr, hostinfo,sf, ip_name_fp, tofile);
	};
	free(hostinfo->names);
  ...
  /***************************************************/
/*  merge /tmp/ip_mac_list and /tmp/ip_name_list into attach_device */
	ip_mac_fp = fopen("/tmp/ip_mac_list", "r");
	ip_name_fp = fopen("/tmp/ip_name_list", "r");
{
	struct stat statbuf;
    stat("/tmp/ip_mac_list", &statbuf);
    if(statbuf.st_size)	
		mergefile(ip_mac_fp, ip_name_fp, dev_fp);
	else
		printf("find none \ n");
}
	if(ip_mac_fp) fclose(ip_mac_fp);
	if(ip_name_fp) fclose(ip_name_fp);
