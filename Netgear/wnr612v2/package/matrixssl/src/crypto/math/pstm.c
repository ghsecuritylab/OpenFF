...
static int32 pstm_mul_2d(pstm_int *a, int32 b, pstm_int *c);
 ...
 */
	a->dp = psMalloc(pool, sizeof (pstm_digit) * PSTM_DEFAULT_INIT);
	if (a->dp == NULL) {
		psError("Memory allocation error in pstm_init\n");
		return PSTM_MEM;
	}
/*
...
/*
	Grow as required
 */
int32 pstm_grow(pstm_int * a, int32 size)
{
	int32			i;
	pstm_digit	*tmp;
 ...
 */
			psError("Memory realloc failed in pstm_grow\n");
			return PSTM_MEM;
		}
/*
...
/*
	...
	called pstm_init_for_read_unsigned_bin first
*/
...
#error Both ENDIAN_LITTLE and ENDIAN_BIG defined.
#endif
	{
		...
		if ((unsigned)c > (PSTM_MAX_SIZE * sizeof(pstm_digit))) {
			int32 excess = c - (PSTM_MAX_SIZE * sizeof(pstm_digit));
			c -= excess;
			b += excess;
		}
...
/*
*/
int32 pstm_count_bits (pstm_int * a)
{
	int32     r;
	pstm_digit q;
...
/*
	Right shift
*/
void pstm_rshd(pstm_int *a, int32 x)
{
	int32 y;
 ...
 */
int32 pstm_lshd(pstm_int * a, int32 b)
{
	int32		x, res;
...
/*
	computes a = 2**b
*/
int32 pstm_2expt(pstm_int *a, int32 b)
{
	int32     z;
...
/*
	c = a mod 2**d
*/
static int32 pstm_mod_2d(pstm_int *a, int32 b, pstm_int *c)
{
	int32 x;
...
/*
	c = a / 2**b
*/
int32 pstm_div_2d(psPool_t *pool, pstm_int *a, int32 b, pstm_int *c,
					pstm_int *d)
{
	pstm_digit D, r, rr;
	int32      x, res;
 ...
 */
int32 pstm_init_copy(psPool_t *pool, pstm_int * a, pstm_int * b, int32 toSqr)
{
	int32 res;
...
*/
	...
	if ((res = pstm_init_size(pool, a, res)) != PSTM_OKAY) {
		return res;
	}
...
*/
static void pstm_reverse (unsigned char *s, int32 len)
{
	int32     ix, iy;
	unsigned char t;
...
{
	int32       ix, iy, iz, tx, ty, pa;
	pstm_digit	c0, c1, c2, *tmpx, *tmpy, *tmpc, *dst;
	pstm_word	t;
	...
	if (paD != NULL) {
		if (paDlen < (sizeof(pstm_digit) * pa)) {
			psTraceCrypto("Error in pa digit size: pstm_mul_comba\n");
			return PS_LIMIT_FAIL;
		}
		dst = paD;
 ...
 */
		...
		/* execute loop */
		for (iz = 0; iz < iy; ++iz) {
			t = (pstm_word)c0 + ((pstm_word)*tmpx++) * ((pstm_word)*tmpy--);
			c0 = (pstm_digit)t;
			t = (pstm_word)c1 + (t >> DIGIT_BIT);
			c1 = (pstm_digit)t; 
			c2 += (pstm_digit)(t >> DIGIT_BIT);
		}
		/* store term */
		dst[ix] = c0;
	}
...
/******************************************************************************/
/*
	Square a large integer
 */
int32 pstm_sqr_comba(psPool_t *pool, pstm_int *A, pstm_int *B,
					 pstm_digit *paD, uint32 paDlen)
{
	int32       pa, ix, iz;
	int32		tx, ty, iy;
	pstm_digit  c0, c1, c2;
	pstm_digit	*tmpy, *tmpx, *tmpc, *dst;
	pstm_word   t, tt; 
	...
	if (paD != NULL) {
		if (paDlen < (sizeof(pstm_digit) * pa)) {
			psTraceCrypto("Error in pa digit size: pstm_sqr_comba\n");
			return PS_LIMIT_FAIL;
		}
		dst = paD;
		memset(dst, 0x0, paDlen);
...
*/
		iy = min(A->used-tx, ty+1);
/*
		Now for squaring tx can never equal ty we halve the distance since they
		approach at a rate of 2x and we have to round because odd cases need
		to be executed
*/
		iy = min(iy, (ty-tx+1)>>1);
		...
		for (iz = 0; iz < iy; iz++) {
			...
			t  = ((pstm_word)*tmpx++) * ((pstm_word)*tmpy--);
		...
		if ((ix&1) == 0) {
			...
			t = c0 + ((pstm_word)A->dp[ix>>1]) * ((pstm_word)A->dp[ix>>1]);
			c0 = (pstm_digit)t;
			t = c1 + (t >> DIGIT_BIT);
