...
#else /* USE_RSA */
			psTraceCrypto("RSA public key algorithm not enabled in cert parse");
			return PS_UNSUPPORTED_FAIL;
#endif /* USE_RSA */
		}
...
*/
		if (cert->certAlgorithm == OID_MD5_RSA_SIG) {
			psMd5Init(&md5Ctx);
			psMd5Update(&md5Ctx, certStart, certLen);
			psMd5Final(&md5Ctx, cert->sigHash);
		} else if (cert->certAlgorithm == OID_SHA1_RSA_SIG) {
			psSha1Init(&sha1Ctx);
			psSha1Update(&sha1Ctx, certStart, certLen);
			psSha1Final(&sha1Ctx, cert->sigHash);
		}
...
void psX509FreeCert(psX509Cert_t *cert)
{
	...
	x509SubjectAltName_t	*active, *inc;
								 ...
								 x509v3extensions_t *extensions)
{
	...
	x509SubjectAltName_t	*activeName, *prevName;
...
{
	psX509Cert_t	*ic, *sc;
	unsigned char	sigOut[10 + SHA1_HASH_SIZE + 5];	/* See below */
...
*/
		...
		if (sc->sigAlgorithm ==  OID_MD5_RSA_SIG ||
			...
			sigLen = 10 + MD5_HASH_SIZE + 8;	/* See above */
		} 
		if (sc->sigAlgorithm == OID_SHA1_RSA_SIG) {
			sigLen = 10 + SHA1_HASH_SIZE + 5;	/* See above */
			sigType = RSA_TYPE_SIG;
		}
#endif /* USE_RSA */
