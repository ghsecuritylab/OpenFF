...
*/
				...
				if (ssl->decryptPriv(pkiPool, ssl->keys->privKey, c,
...
*/
		...
		if (certMatch == 0) {
			...
			psTraceInfo("Can only support RSA_SIGN cert authentication\n");
			return MATRIXSSL_ERROR;
		}
		certChainLen = *c << 8; c++;
		certChainLen |= *c; c++;
