...
#define CONN_ITER			100 /* number of connections per 'type'of hs */
...
#if defined(USE_FILE_SYSTEM) && !defined(USE_HEADER_KEYS)
...
#endif  /* USE_FILE_SYSTEM && !USE_HEADER_KEYS */

#ifdef USE_HEADER_KEYS
#include "privkeySrv.h"
#include "certSrv.h"
#include "CAcertSrv.h"
...
#endif /* ENABLE_PERF_TIMING */		
		if (initializeHandshake(clnConn, svrConn, ciphers[id], &sid) < 0) {
...
/*
		Confirm handshake is complete.
*/
		if (sdrc == MATRIXSSL_HANDSHAKE_COMPLETE) {
			return PS_SUCCESS;
		} else {
			return PS_FAILURE;
		}
...
*/
		...
		printf("Unexpected error in performHandshake\n");
		return PS_FAILURE;
	}
...
*/
	...
	if (rc == MATRIXSSL_REQUEST_RECV) {
		...
		printf("Unexpected error in exchangeAppData\n");
		return PS_FAILURE;
	}
...
static int32 initializeServer(sslConn_t *conn, testCipherSpec_t cipherSuite)
{
	...
	if (conn->keys == NULL) {
		...
		if (cipherSuite.rsa) {
#if defined(USE_FILE_SYSTEM) && !defined(USE_HEADER_KEYS)	
			if (matrixSslLoadRsaKeys(keys, svrCertFile, svrKeyFile, NULL, NULL) < 0) {
				return PS_FAILURE;
			}
#endif /* USE_FILE_SYSTEM && !USE_HEADER_KEYS */
