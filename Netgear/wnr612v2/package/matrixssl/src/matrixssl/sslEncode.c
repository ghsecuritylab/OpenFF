...
/******************************************************************************/
/*
	Encode the incoming data into the same buffer for sending to remote peer.
...
/*
	Validate the plaintext buffer is ssl->recordHeadLen bytes in front
	of the buffer
*/
	pad = ssl->recordHeadLen;
	

	if (buf + pad != ptBuf) {
		psTraceInfo("Input buffers not correct to matrixSslEncode\n");
		return PS_MEM_FAIL;
	}
	c = buf;
	end = buf + size;
 ...
 */
	...
	if ((rc = encryptRecord(ssl, SSL_RECORD_TYPE_APPLICATION_DATA,
			messageSize, padLen, encryptStart, &tmpout, &c)) < 0) {
		return rc;
	}
	*len = c - buf;
...
/******************************************************************************/
/*
	A helper function for matrixSslEncode to determine the correct
	destination size before allocating an output buffer. 
*/
int32 matrixSslGetEncodedHeaderSize(ssl_t *ssl)
{
	int32	size;
	
	size = ssl->recordHeadLen;
	return size;
}
...
{	
	len += matrixSslGetEncodedHeaderSize(ssl);
	if (ssl->flags & SSL_FLAGS_WRITE_SECURE) {
		len += ssl->enMacSize;
		/* Leave this pad line on its own */
...
/*
				No client auth support. Send a no_certificate warning
*/
...
*/
			messageSize +=
				...
				1 + /* change cipher spec */
				MD5_HASH_SIZE + SHA1_HASH_SIZE + /* SSLv3 finished */
						   ...
						   sslBuf_t *out, unsigned char **c)
{
#ifdef USE_TLS
		if (type == SSL_RECORD_TYPE_HANDSHAKE) {
			sslUpdateHSHash(ssl, encryptStart, (uint32)(*c - encryptStart));
		}
		*c += ssl->generateMac(ssl, (unsigned char)type, encryptStart,
			(uint32)(*c - encryptStart), *c);
#else /* USE_TLS */
	if (type == SSL_RECORD_TYPE_HANDSHAKE) {
	...
	*c += sslWritePad(*c, (unsigned char)padLen);
	...
	if (ssl->encrypt(&ssl->sec.encryptCtx, encryptStart, encryptStart, 
			(uint32)(*c - encryptStart)) < 0 || *c - out->end != messageSize) {
...
*/	
	...
	if (psGetEntropy(ssl->sec.serverRandom + 4, SSL_HS_RANDOM_SIZE - 4) < 0) {
		return MATRIXSSL_ERROR;
	}
/*
...
/*
	Write out the certs
*/
	*c = (unsigned char)(((totalCertLen + (lsize - 3)) & 0xFF0000) >> 16); c++;
...
*/
	if (cipherSpec == 0) {
		...
		if (sslGetCipherSpec(ssl, cipherSpec) == NULL) {
...
*/	
	if (ssl->myVerifyDataLen != 0) {
		extLen = 2 + ssl->myVerifyDataLen + 5; /* 2 total len, 5 type/len/len */
	}
#endif /* ENABLE_SECURE_REHANDSHAKES */
...
*/
	...
	if ((rc = psGetEntropy(ssl->sec.clientRandom + 4, SSL_HS_RANDOM_SIZE - 4))
			< PS_SUCCESS) {
		return rc;
	}
/*
...
*/
	if (cipherSpec == 0) {
		if ((rc = sslGetCipherSpecList(c, (int32)(end - c), addScsv)) < 0) {
			return SSL_FULL;
		}
		c += rc;
	} else {
...
*/
			...
			if (psGetEntropy(ssl->sec.premaster + 2,
			...
			if (ssl->encryptPub(pkiPool, &ssl->sec.cert->publicKey,
					ssl->sec.premaster, ssl->sec.premasterSize, c,
					(uint32)(end - c)) != (int32)keyLen) {
				return MATRIXSSL_ERROR;
			}
			c += keyLen;
