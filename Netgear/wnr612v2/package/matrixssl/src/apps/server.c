 ...
 */
	...
	if (FD_ISSET(lfd, &readfd)) {
		for (acceptSanity = 0; acceptSanity < GOTO_SANITY; acceptSanity++) {
			...
			if ((rc = matrixSslNewServerSession(&cp->ssl, keys, NULL)) < 0) {
		...
		if ((len = matrixSslGetOutdata(cp->ssl, &buf)) > 0) {
			...
			if (transferred <= 0) {
				...
				if (rc == MATRIXSSL_REQUEST_SEND || transferred < len) {
					if (likely(wSanity++ < GOTO_SANITY)) goto WRITE_MORE;
				}
			}
		} else if (len < 0) {
			closeConn(cp, PS_ARG_FAIL);
...
PROCESS_MORE:
			...
			switch (rc) {
				...
				case MATRIXSSL_REQUEST_RECV:
					if (likely(rSanity++ < GOTO_SANITY)) goto READ_MORE; 
					break;
