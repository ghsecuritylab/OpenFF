...
//dvd.chen modify for DNI_DIAL_ON_DEMAND
#if 1  //DNI_DIAL_ON_DEMAND
	fp = fopen("/etc/xl2tpd/options", "r");
	if (fp == NULL) {
		l2tp_log(LOG_ERR, "Unable to open %s\n", "/etc/xl2tpd/options");
		//exit(1);
		return -EINVAL;
	}
	//l2tp_log (LOG_WARNING, "##ppp options:\n");
	while (fgets(opt_buf, sizeof(opt_buf)-1, fp) != NULL)
	{
		//l2tp_log (LOG_WARNING, "%s, len[%d]\n", opt_buf, strlen(opt_buf));
		if( !strcmp(opt_buf, "demand\n") )
			is_demand = 1;
	}
	fclose(fp);
	l2tp_log (LOG_WARNING, "\n%s::is_demand[%d]\n", __FUNCTION__, is_demand);
#if 1  //DO_NOT_KILL_PPPD
	//get pid from /var/run/ppp0.pid here
	if( is_demand )
	{
		if( (fd = open( "/var/run/ppp0.pid", O_RDONLY)) != -1 )
		{
			...
			if( count > 0 )
			{
				buf[count] = '\0';
				c->pppd = atoi(buf);
				l2tp_log(LOG_DEBUG, "##%s:: c->pppd[%d]\n", __FUNCTION__, c->pppd);
				close(fd);
			}
			else
			{
				l2tp_log(LOG_ERR, "%s:: ppp0.pid contains nothing\n", __FUNCTION__);
				close(fd);
				return -EINVAL;
			}
		}
		...
		{
			l2tp_log(LOG_WARNING, "Unable to open %s, pppd was dead...\n", "/var/run/ppp0");
			is_pppd_dead = 1;
		}
	}
#else
	//demand mode and not demandex == > kill the existed pppd fist, and then fall through tp spawn a new one.
	if( is_demand && !is_demandex)  
	{
		l2tp_log(LOG_DEBUG, "##%s:: ppp demand mode, gonna kill the existed pppd first, c->pppd[%d]\n",
			__FUNCTION__, c->pppd);
		stropt[pos] = (char *) malloc (strlen("demandex") + 1);
		strncpy (stropt[pos], "demandex", strlen("demandex") + 1);
        	pos++;
		system("killall -1 pppd");
		sleep(2);
		system("killall -9 pppd");
		sleep(2);  //sleep a while for kernel ppp to free ifunit 0, otherwise it is possible to get a ifunit=1.
	}
#endif  //DO_NOT_KILL_PPPD
...
#if 1  //DO_NOT_KILL_PPPD
    if( !is_demand && c->pppd > 0) //check pppd's pid only on persist mode
#else
    if (c->pppd > 0)
#endif
    {
    ...
    if (kernel_support)
    {
       ...
       stropt[pos] = NULL;
#if 1  //DO_NOT_KILL_PPPD
	if( is_demand && !is_pppd_dead)
	{	
		//open a Unix Domain Socket to send created pppox socket (fd2) to the recved peer
		int s;
		...
		if ( s == -1 )
		{
			l2tp_log(LOG_WARNING, "##%s::Create Unix socket failed\n", __FUNCTION__);
		...
		//for Unix Domain Socket, it always needs to connect first for later sneding/receiving message.
		while(try_count++ < 3)
		{
			if (connect(s, (struct sockaddr *) &a_srvr, addrLength) < 0) 
			{		
				l2tp_log(LOG_ERR, "##%s::connect failed(%d), errno[%d]:%s\n", __FUNCTION__, 
		...
		if(try_count >=3)
		{
			l2tp_log(LOG_ERR, "##%s::tried connect for [%d] times still failed\n", __FUNCTION__, try_count-1);
			close(s);
			return -EINVAL;
		}
	
		if( send_fd(s, fd2, NULL, 0) == 0)
		{
			l2tp_log(LOG_WARNING, "##%s:: pppox socket fd sent successfully, fd[%d]\n", __FUNCTION__, fd2);
			close(s);
		}
		else
		{
			l2tp_log(LOG_ERR, "##%s:: send_fd() failed, errno[%d]\n", __FUNCTION__, errno);
			close(s);
			return -EINVAL;
		}		
	}
...
#if 1  //DO_NOT_KILL_PPPD
	if( is_demand && !is_pppd_dead)
	{	//we always have kernel_support, so won't be here.
		l2tp_log (LOG_ERR, "%s:: You are not using kernel_support, please check it.\n", __FUNCTION__);
		return -EINVAL;
		//fatal("kernel_support");
	}
#endif  //DO_NOT_KILL_PPPD
    }
...
#if 1  //DNI_DIAL_ON_DEMAND
	else if (! strncmp(argv[i],"-demand",7)) {
		//l2tp_log(LOG_INFO, "%s: setting gconfig.dial_on_demand to [%d]\n",__FUNCTION__, 1);
		gconfig.dial_on_demand=1;
	}
#endif
        else {
            usage();
