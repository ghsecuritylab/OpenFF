...
void destroy_call (struct call *c)
{
    ...
    struct timeval tv;
    pid_t pid;
l2tp_log (LOG_DEBUG, "##%s, c->fd[%d], c->pppd[%d]\n", __FUNCTION__, c->fd, c->pppd);
    /*
     * Close the tty
     */
    if (c->fd > 0)
...
/*	if (c->dethrottle) deschedule(c->dethrottle); */
    ...
    //Modified by dvd.chen, for dial-on-demand
    //if (pid)
#if 1  //DNI_DIAL_ON_DEMAND
    l2tp_log (LOG_DEBUG, "%s::gconfig.dial_on_demand[%d], pppd_pid[%d]\n", __FUNCTION__, gconfig.dial_on_demand, pid);	
    if( gconfig.dial_on_demand==0 && pid )  //don't kill pppd on dial-on-demand mode
#endif
    {
...
#if 0  //DNI_PERSIST_REDIAL
	l2tp_log (LOG_DEBUG, "##%s::L2TP persist mode, gonna redial after 20 seconds...\n", __FUNCTION__);
	system("/etc/rc.d/l2tp_redial.sh &");
#endif
    }
    if (c->container)
    {
...
l2tp_log(LOG_DEBUG, "##Enter new_call\n");
    if (!tmp)
        return NULL;
    tmp->tx_pkts = 0;
    tmp->rx_pkts = 0;
