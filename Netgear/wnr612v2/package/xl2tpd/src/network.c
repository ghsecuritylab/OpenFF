...
void control_xmit (void *b)
{
    ...
    else
    {
         ...
         */
        //modified by dvd.chen, it is too quickly to consume the retry count,
        //while l2tp is restarting, seems it needs some more time for kernel to be prepared and
        //be able to pass up the received SCCRP packet.
        //tv.tv_sec = 1;
        tv.tv_sec = (buf->retries < 3) ? buf->retries:3;
	 //end of dvd.cehn
        tv.tv_usec = 0;
        schedule (tv, control_xmit, buf);
#ifdef DEBUG_CONTROL_XMIT
        l2tp_log (LOG_DEBUG, "%s: Scheduling and transmitting packet %d, retry[%d]\n",
