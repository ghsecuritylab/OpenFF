...
#if 0  //turn this on for debug
#define DBG_LOG(fmt, args...) do { \
	FILE *fp = fopen("/dev/console", "r+"); \
	fprintf(fp, fmt, ##args); \
	fclose(fp); \
} while (0);
#else
...
#if 1 //DNI_DIAL_ON_DEMAND
extern bool	demand;  //in options.c	
//extern bool	demandex;  //in options.c	
#endif
...
DBG_LOG("##%s:: device_got_set[%d], demand[%d]\n", __FUNCTION__, device_got_set, demand);
	if (device_got_set)
		return 0;
	...
	{
		DBG_LOG("##%s:: L2TP demand mode, don't check if pppol2tp_fd valid\n", __FUNCTION__);
	}
#endif
...
#if 1 //DNI_DIAL_ON_DEMAND
	if( demand && (pppol2tp_fd == -1) ) 
	{
		//we have to signal xl2tpd to connect pppox socket again 
		DBG_LOG("##%s::ppp demand mode, try to wake up xl2tpd to reconnect now...\n", __FUNCTION__);
		system("echo \"c client\" > /var/run/xl2tpd/l2tp-control &");  //start l2tp connection process
		//sleep(3);
#if 1  //DO_NOT_KILL_PPPD
		//wait for a while for xl2tp to re-create the pppox socket and write it to /tmp/pppol2tp_fd
		//if(pppol2tp_fd == -1)   //means the tunnel and session had been closed last time on demand mode
		{
			int s,c;
			...
			//open a unix domain socket to receive the pppox socket fd created by xl2tpd in start_pppd()
			// Create a Unix Socket:
			s = socket(PF_UNIX, SOCK_DGRAM,0);    
			if ( s == -1 )
			{
				DBG_LOG("##%s::Create Unix socket failed\n", __FUNCTION__);
				return -1;  /* Failed: check errno */
			}

#if 0
			//Now make certain that we can receive credentials on this socket:     
			if( setsockopt(s, SOL_SOCKET, SO_PASSCRED, &b, sizeof b) != 0)    
				DBG_LOG("##%s::setsockopt failed\n", __FUNCTION__);
#endif

			//Create the abstract address of  the socket server :
			memset(&a_srvr,0,sizeof a_srvr);    
			a_srvr.sun_family = AF_UNIX;    
			sprintf(a_srvr.sun_path, "/tmp/zSOCKET-SERVER");    
			addrLength = sizeof(a_srvr.sun_family) + strlen(a_srvr.sun_path);
			a_srvr.sun_path[0] = 0;
	
			if (bind(s, (struct sockaddr *) &a_srvr, addrLength) < 0) 
			{		
				DBG_LOG("##%s::Could not Bind socket\n", __FUNCTION__);	
				fatal("bind failed\n");
			}
			else
				DBG_LOG("##%s socket bind success\n", __FUNCTION__);
				...
				if (ret <= 0)
			       {
			            if (ret == 0)
			            {
			               DBG_LOG ( "##%s: select timeout, try_count[%d]\n", __FUNCTION__, try_count);
			            }
			            else
			            {
			                    DBG_LOG ("##%s:: select returned error %d (%s)\n",
			                        __FUNCTION__, errno, strerror (errno));
						fatal("select return error\n");
			            }
			            //fatal("select faled or timeout\n");
			       }
				else
				{
					DBG_LOG("##%s:: slect succeed, try_count[%d], ret[%d]\n", __FUNCTION__, try_count, ret);
					break;
				}
			}
			...
			if(try_count >= 3)
			{
				DBG_LOG("##%s:: tried for [%d] times of select still failed\n", __FUNCTION__, try_count);
				close(s);
				fatal("select failed\n");
			}
			...
			{
				if( (pppol2tp_fd = recv_fd(s)) == -1 )
				{
					DBG_LOG("##%s:: recv_fd() failed, errno[%d]: %s\n", __FUNCTION__, errno, strerror(errno));
					close(s);
					fatal("recv_fd() failed");
				}
				else
				{
					close(s);
					unlink("/tmp/zSOCKET-SERVER");
					DBG_LOG("##%s:: pppox socket fd recved, pppol2tp_fd[%d]\n", __FUNCTION__, pppol2tp_fd);
				}
			}
			else
			{
				DBG_LOG("##%s:: recv_fd() failed\n", __FUNCTION__);
				close(s);
				fatal("recv_fd failed\n");
			}
		}

		if(getsockname(pppol2tp_fd, (struct sockaddr *)&so, &len) < 0) {
			fatal("Given FD for PPPoL2TP socket invalid (%s)", strerror(errno));
		}

		if(so.pppol2tp.sa_family != AF_PPPOX) {
			fatal("Socket of not a PPPoX socket, socket-family-is[%d]", so.pppol2tp.sa_family);
		}
...
#endif  //DNI_DIAL_ON_DEMAND

	if(pppol2tp_fd == -1) {
		fatal("No PPPoL2TP FD specified");
	}
DBG_LOG("##%s::pppol2tp_fd[%d]\n", __FUNCTION__, pppol2tp_fd);
	return pppol2tp_fd;
}
...
static void disconnect_pppol2tp(void)
{
	if (pppol2tp_fd >= 0) {
		close(pppol2tp_fd);
		pppol2tp_fd = -1;
		DBG_LOG("##%s, pppol2tp_fd closed, signal xl2tp to close tunnel as well\n", __FUNCTION__);
#if 1 //DNI_DIAL_ON_DEMAND
...
#if 1 //DNI_DIAL_ON_DEMAND
	DBG_LOG("##%s: demand[%d], mru[%d]\n", __FUNCTION__, demand, mru);
	if( !demand )
#endif
