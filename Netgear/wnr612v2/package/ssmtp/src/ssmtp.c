...
#define TMP_REVALIASES_FILE "/tmp/ssmtp/revaliases"
#define TMP_CONFIGURATION_FILE "/tmp/ssmtp/ssmtp.conf"
#endif
...
void revaliases(struct passwd *pw)
{
	...
	if((fp = fopen(TMP_REVALIASES_FILE, "r"))) {
...
bool_t read_config()
{
	...
	if(config_file == (char *)NULL) {
		config_file = strdup(TMP_CONFIGURATION_FILE);
		if(config_file == (char *)NULL) {
...
static int32 certCb(ssl_t *ssl, psX509Cert_t *cert, int32 alert)
{
	...
	if (alert > 0) {
		...
		printf("Certificate callback returning fatal alert\n");
		return alert;
	}
...
//tim11
	...
	if (matrixSslNewKeys(&keys) < 0) {
		printf("MatrixSSL library key init failure.  Exiting\n");
		return -1;
	}
	...
	rc=connect(s, (struct sockaddr *)&name, namelen);
	...
	//blocking again
	fcntl(s, F_SETFL, fcntl(s, F_GETFL, NULL) &(~O_NONBLOCK)); 
#endif
	...
	if (rc != MATRIXSSL_REQUEST_SEND) {
		printf("New Client Session Failed: %d.  Exiting\n", rc);
		return -1;
	}
#else
		ssl = SSL_new(ssl);
...
WRITE_MORE:
              len = matrixSslGetOutdata(ssl, &tmpbuf);
              if(len>0)
              {
                      transferred=send(fd,tmpbuf,len,0);
                      if(transferred >0)
                      {
	                       if ((rc = matrixSslSentData(ssl, transferred)) < 0) 
	                       {
                   		 	  printf("##matrixSslSentData <0 \n");
					  return NULL;				
	                       }
...
READ_MORE:
              if ((len = matrixSslGetReadbuf(ssl, &tmpbuf)) <= 0) 
		{
	   	       printf("Matrix err:  matrixSslGetReadbuf\n");
 		       return NULL;
	       }	   
       	if ((transferred = recv(fd, tmpbuf, len, 0)) < 0) 
		{
		       printf("Matrix err:  recv\n");
		       return NULL;
       	}
	       if (transferred == 0) 
		{
		       printf("Matrix err:  transferred=0\n");
		       return NULL;
	       }

               rc = matrixSslReceivedData(ssl, (int32)transferred, &tmpbuf,	(uint32*)&len);
       	 if(rc < 0)
	        {
	              printf("Matrix err:  matrixSslReceivedData\n");
	       	return NULL;
	        }
...
PROCESS_MORE:
		...
		case MATRIXSSL_REQUEST_RECV:
	       ...
	       {
                     //lol
			strncpy(buf,"200 STARTTLS HS done",20);
			return buf;
		}
...
#ifdef DNI_MATRIX_SSL
int helo_srv_md5(int sock,int * server_md5,char *response)
{
	char *ex;
	...
	if(use_tls == True) 
      { 
      		if(fd_gets(response, BUF_SZ, sock) == NULL) {
			return(0);
		}
              ex=strstr(response,"CRAM-MD5");
	       if(ex != NULL)
		   	*server_md5=1;
      	}
	else
...
ssize_t fd_puts(int fd, const void *buf, size_t count) 
{
	...
	if(use_tls == True) { 
#ifdef DNI_MATRIX_SSL
              int32		 len,transferred,rc,available;
              unsigned char *oldbuf;

WRITE_MORE:
        len = matrixSslGetOutdata(ssl, &oldbuf);
        if(len>0)
        {
                transferred=send(fd,oldbuf,len,0);
                if(transferred >0)
                {
	                 if ((rc = matrixSslSentData(ssl, transferred)) < 0) 
	                 {
	...
	if ((available = matrixSslGetWritebuf(ssl, &oldbuf,	strlen((char *)buf) + 1)) < 0) 
	{
              printf("##not available\n");
		return PS_MEM_FAIL;
	}
	strncpy((char*)oldbuf,buf,available);
	if ((rc=matrixSslEncodeWritebuf(ssl, strlen((char *)buf)) )< 0) 
	{
              printf("##encode fail,rc=%d\n",rc);
             	return PS_MEM_FAIL;
       }
				             	
	while((len = matrixSslGetOutdata(ssl, &oldbuf))>0)
	{
	       transferred = send(fd, oldbuf, len, 0);
       	if ((rc = matrixSslSentData(ssl, transferred)) < 0) 
	      	{
...
#else
       if(helo_srv_md5(sock,&server_md5,buf)!=2)
	{
		die("%s (%s)", buf, hostname);
	}
