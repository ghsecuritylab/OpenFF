					  ...
					  u16 ver)
{
    ...
    /* wrong passwd detection */
    sm->wpa_4way_status = WPA_M2_SENT;
					    ...
					    const u8 *addr, int secure)
{
    ...
    /* catch all for all races where wpa_supplicant belives
     * it is connected to BSSID 0:0:0:0:0:0 
     */
	if (memcmp(sm->bssid, "\x00\x00\x00\x00\x00\x00", ETH_ALEN) == 0) {
		wpa_printf(MSG_WARNING, "WPA/RSN: AP BSSID is 0\n");
		printf( "WPA/RSN: AP BSSID is 0\n");
    }
    ...
    /* wrong passwd detection */
    sm->wpa_4way_status = WPA_ALL_COMPLETE;
    wpa_sm_recerr(sm,0);
	if (secure) {
					  ...
					  u16 ver)
{
    ...
    /* wrong passwd detection */
    sm->wpa_4way_status = WPA_M3_RECD;
    ...
    /* wrong passwd detction */
    sm->wpa_4way_status = WPA_M4_SENT;
	...
	if (key_info & WPA_KEY_INFO_KEY_TYPE) {
		...
		if (peerkey) {
            ...
            if (key_info & WPA_KEY_INFO_MIC) {
                /* 3/4 4-Way Handshake */
                if (wpa_sm_get_state(sm) > WPA_ASSOCIATED )  {
                    wpa_supplicant_process_3_of_4(sm, key, ver);
                    printf("WPA: EAPOL-Key (3/4) "
                            "Received in GOOD state current state %d expected state  is %d", wpa_sm_get_state(sm), WPA_4WAY_HANDSHAKE);
                } else {
                    printf("WPA: EAPOL-Key (3/4) "
                            "Received in bad state Dropped current state %d expected state  is %d", wpa_sm_get_state(sm), WPA_4WAY_HANDSHAKE);
                    wpa_printf(MSG_WARNING, "WPA: EAPOL-Key (3/4) "
                            "Received in bad state Dropped current state %d expected state  is %d", wpa_sm_get_state(sm), WPA_4WAY_HANDSHAKE);
                }
            } else {
                /* 1/4 4-Way Handshake */
		...
		if (key_info & WPA_KEY_INFO_MIC) {
            ...
            if ((wpa_sm_get_state(sm) == WPA_GROUP_HANDSHAKE) ||
                    (wpa_sm_get_state(sm) == WPA_COMPLETED)) {
                wpa_supplicant_process_1_of_2(sm, src_addr, key,
						      extra_len, ver);
            } else {
			wpa_printf(MSG_WARNING, "WPA: EAPOL-Key (Group) "
				   "Received in bad state Dropped %d COMPLETED is %d", wpa_sm_get_state(sm), WPA_COMPLETED);
            }
		} else {
			wpa_printf(MSG_WARNING, "WPA: EAPOL-Key (Group) "
...
int wpa_sm_errfile_init(char *ifname)
{
    wpa_printf(MSG_DEBUG, "WPA: %s:Error file Init for interface %s\n", __func__,  ifname);
    os_rec_err(ifname,0);

}
int wpa_sm_recerr(struct wpa_sm *sm, int status)
{
    wpa_printf(MSG_DEBUG, "WPA:%s Calling rec error with status %d\n", sm->ifname,status );
    if (status == 0) os_rec_err(sm->ifname, 0);
    else if (sm->wpa_4way_status == WPA_M2_SENT) os_rec_err(sm->ifname, status);
}
