...
#endif /* CONFIG_PEERKEY */
	case EVENT_CONFIGME:
		params.filter_bssid_flag = 0;
		params.filter_bssid = NULL;
		params.filter_ssid_length = 0;
		params.filter_ssid = 0;
		wpa_s->conf->wps->reg_mode = WPS_SUPPLICANT_REGMODE_NONE;
		wpa_s->conf->wps->do_save = 1;
		params.dev_pwd = (u8 *)"00000000"; /* for push button method */
		params.dev_pwd_len = 8;
		wpa_printf(MSG_INFO, "WPS push button pressed for client VAP");
		if (eap_wps_enable(wpa_s, wpa_s->conf->wps, &params)) {
			wpa_printf(MSG_ERROR, "Push button failed to enable Client VAP WPS");
		}
		break;
	default:
