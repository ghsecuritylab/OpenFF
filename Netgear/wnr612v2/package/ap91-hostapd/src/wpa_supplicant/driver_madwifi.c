...
wpa_driver_madwifi_scan(void *priv, const u8 *ssid, size_t ssid_len)
{
	 ...
	 */
     /* Ind repeater NTGR 
      * Increase scan timeout so that AP has time to scan all the
      * channels - the time required is more now since the hops
      * are repeated as  home 500ms - away 100 - home 500 ms.
      */
	eloop_register_timeout(50, 0, wpa_driver_wext_scan_timeout, drv->wext,
			       drv->ctx);
