				      ...
				      char *buf, size_t buflen)
{
	...
	if (wps) {
		...
		if (wps->ap_setup_locked) {
			ret = os_snprintf(pos, end - pos, "wps_ap_setup_locked=Locked\n");
			if (ret < 0 || ret >= end - pos)
				return pos - buf;
			pos += ret;
		} else {
			ret = os_snprintf(pos, end - pos, "wps_ap_setup_locked=Unlocked\n");
			if (ret < 0 || ret >= end - pos)
				return pos - buf;
			pos += ret;
		}
        ...
        struct hostapd_data *hapd, char *cmd)
{
        ...
        char *filepath;
        int fd;
        unsigned char buf[1500]; /* more than big enough */
        int len;
        ...
        for (;;) {
                ...
                if (*value == '-') {
                        ...
                        if (!strcmp(value, "-norestart"))
                            ...
                            wpa_printf(MSG_ERROR, "Unknown option for CONFIGIE: %s", value);
                            return -1;
                        }
                } else {
                        /* got filepath */
        ...
        close(fd);
        if (len <= 0) {
                    wpa_printf(MSG_ERROR, "CONFIGIE failed to read file %s", filepath);
                    return -1;
        }

        if (wps_set_ssid_configuration(hapd, buf, len, !nosave, !norestart, NOT_AP_PIN)) {
