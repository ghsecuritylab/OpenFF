	...
	if (!enabled) {
    	...
    	*/
		...
		if(conf->auth_algs==1 && wep.keys_set==0) {
			...
			set80211param(priv, IEEE80211_PARAM_AUTHMODE,
#ifdef NTG_DUT_NOT_SUPPORT_WEP_OPEN_AUTH
			IEEE80211_AUTH_AUTO
#else
			IEEE80211_AUTH_OPEN
#endif
				       ...
				       char *custom)
{
	...
	else if (strncmp(custom, "WPS.indication", 14) == 0) {
		if(0==strncmp(&custom[15],"pin=",4)) {
			char pwd[8];
			strncpy(pwd,&custom[19],8);
			struct eap_wps_enable_params params = {};
			params.dev_pwd = (u8 *)pwd;
			params.dev_pwd_len = 8;
			params.config_who = WPS_CONFIG_WHO_ME;
			params.seconds_timeout = 240; /* Client PIN walktime is 240s in NETGEAR spec. */
			wpa_printf(MSG_INFO, "WPS client pin for if:%s, pin=%s",drv->iface, params.dev_pwd);
			if (eap_wps_enable(hapd, hapd->conf->wps, &params)) 
				wpa_printf(MSG_ERROR, "Client pin failed to enable WPS");
		}
		else if(0==strncmp(&custom[15],"stop",4)) {
			if (hapd->conf->wps->wps_job_busy) {
				wpa_printf(MSG_INFO, "Stop WPS for if:%s",drv->iface);
				system("env -i ACTION=SET_STATE WPS_STATE=5 PROTO=static /sbin/hotplug-call wps");
				if(eap_wps_disable(hapd, hapd->conf->wps, eap_wps_disable_reason_user_stop))
					wpa_printf(MSG_ERROR, "Failed to stop WPS");
			}
		}
		else {
			int i;
			printf("Interface: %s\n\tbssid="MACSTR"\n",hapd->conf->iface,MAC2STR(hapd->own_addr));
			printf("\tssid=%s\n\tencription=",hapd->conf->ssid.ssid);
			if (hapd->conf->wpa) {
				...
				if (hapd->conf->ssid.wep.keys_set) {
					printf("WEP\n\tWEP_Key%i=0x",hapd->conf->ssid.wep.idx);
					for (i = 0; i < hapd->conf->ssid.wep.len[hapd->conf->ssid.wep.idx]; i++ )
						printf("%02X",hapd->conf->ssid.wep.key[hapd->conf->ssid.wep.idx][i]);
				}
				else
					printf("NONE");
			}
			...
			else if (hapd->conf->auth_algs & HOSTAPD_AUTH_SHARED_KEY)
				printf("SHARED");
			else
				printf("OPEN");
			if (hapd->conf->eap_user->password_len > 64 || hapd->conf->eap_user->password_len <= 0 )
				printf("\n\tinvalid password_len %d",hapd->conf->eap_user->password_len);
			else {
				printf("\n\tpassword=0x");
				for (i = 0; i < hapd->conf->eap_user->password_len; i++ )
					printf("%02X",hapd->conf->eap_user->password[i]);
			}
			printf("\n");
			if (hapd->conf->ssid.wpa_psk) {
				printf("\tpsk=0x");
				for (i = 0; i < PMK_LEN; i++ )
					printf("%02X",hapd->conf->ssid.wpa_psk->psk[i]);
				printf("\n");
			}
			if (hapd->conf->wps) {
				printf("\twps_configured=");
				if (WPS_WPSSTATE_UNCONFIGURED == hapd->conf->wps->wps_state)
					printf("Unconfigured");
				else
					printf("Configured");
				printf("\n\twps_ap_setup_locked=");
				if (hapd->conf->wps->ap_setup_locked)
					printf("Locked");
				else
					printf("Unlocked");
				printf("\n\tselected_registrar=");
				if (hapd->conf->wps->selreg)
					printf("TRUE");
				else
					printf("FALSE");
				printf("\n\tdev_pwd_id=");
				switch (hapd->conf->wps->dev_pwd_id) {
					...
					case WPS_DEVICEPWDID_MACHINE_SPEC:
						printf("Machine-specified\n");
						break;
					case WPS_DEVICEPWDID_PUSH_BTN:
						printf("PushButton\n");
						break;
					case WPS_DEVICEPWDID_REG_SPEC:
						printf("Registrar-specified\n");
						break;
					default:
						printf("%i\n",hapd->conf->wps->dev_pwd_id);
						break;
				}
			}
		}
...
madwifi_init(struct hostapd_data *hapd)
{
	...
	do {
		...
		//Try to get hw_mode (11a or 11g);
		if (ioctl(drv->ioctl_sock, SIOCGIWNAME, &iwr) >= 0)
			if(strchr(iwr.u.name,'a') != NULL)
        ...
        )
{
	...
	struct ieee80211_frame *wh;     /* the header */
        ...
        for (ie = (u8 *)&wh[1]; ie <= endfrm-2 && 
                        ...
                        ie = next_ie) {
                if (ie_len >= 6 && ie[0] == IEEE80211_ELEMID_VENDOR && 
			        ...
			        ie[2] == 0x00 && ie[3] == 0x50 && ie[4] == 0xF2 && ie[5]==0x04) {
                        ...
                        eap_wps_handle_mgmt_frames(drv->hapd, src_addr, 
                                buf, len, ie+6, ie_len-6);
						sta = ap_get_sta(drv->hapd, src_addr);
						if (sta)
...
#endif /* EAP_WPS */
        .set_iface_flags        = madwifi_set_iface_flags,
};
