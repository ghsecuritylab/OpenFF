...
static void hostapd_config_defaults_bss(struct hostapd_bss_config *bss)
{
	...
	bss->eap_reauth_period = 3600;
       //tim11:change default rekey interval to 3600
	bss->wpa_group_rekey = 3600;
...
{
	//tim11: fix wmm default parameters
        ...
        )
{
	...
	if (bss->auth_algs == 0) {
		...
		bss->ieee80211w = atoi(value);
        ...
        } else if (strcmp(tag, "wps_ap_setup_locked") == 0) {
                bss->wps->conf_ap_setup_locked = bss->wps->ap_setup_locked =
                                                 atol(value);
#endif
	        ...
	        if (hexstr2bin(value, bss->wps->dev_oui, 4) ||
                ...
                if ((bss->wps->upnp_desc_url = strdup(value)) == NULL)
                        goto malloc_failure;
        #endif /* USE_INTEL_SDK */
#ifdef NETGEAR_WPS_UPNP_SETTING
        } else if (strcmp(tag, "wps_upnp_ad_period") == 0) {
                bss->wps->upnp_ad_period = atol(value);
        } else if (strcmp(tag, "wps_upnp_ad_ttl") == 0) {
                bss->wps->upnp_ad_ttl = atol(value);
#endif		
        #ifdef WPS_OPT_NFC
...
void hostapd_config_free(struct hostapd_config *conf)
{
	...
	free(conf->bss);
       ...
       //tim11mem_2010may
	free(conf->supported_rates);
       free(conf->basic_rates);
       //
	free(conf);
}
