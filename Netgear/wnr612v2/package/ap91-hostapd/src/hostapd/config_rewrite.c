        ...
        )
{
    ...
    if (section_name == NULL) {
        if ((tag_equal(line, "wpa=1")) || (tag_equal(line, "wpa=2")) || (tag_equal(line, "wpa=3"))) {
            configedit_rewind(bound);
            config_rewrite_remove_others(bound, "wep_key0");
            configedit_rewind(bound);
            config_rewrite_remove_others(bound, "wep_key1");
            configedit_rewind(bound);
            config_rewrite_remove_others(bound, "wep_key2");
            configedit_rewind(bound);
            config_rewrite_remove_others(bound, "wep_key3");
            configedit_rewind(bound);
            config_rewrite_remove_others(bound, "wep_default_key");
        }
    }
