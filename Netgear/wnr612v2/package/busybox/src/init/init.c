...
#define DNI_RESET_BUTTON_HDL
#define DEV_NAME_GPIO	"ar531xgpiointr"
/* #define AP81 */
...
#ifdef DNI_RESET_BUTTON_HDL
static void push_reset_button_signal(int sig);
#endif
...
/* 
	receving a signal to remind that the device upgrade firmware. 
	when device is upgrading, the reset button should inactive.
*/
static int webupgrade=0;
static void set_webupgrade_signal(int sig)
{
	webupgrade=1;
}
...
static void push_reset_button_signal(int sig)
{
	...
	static unsigned long up_time;
	static unsigned char press_down = 0;
	
	if((i = read(gpiointr_fd, buffer, 1)) != 1){
		printf("read gpiointr error\n");
		return;
	}
	buffer[i] = '\0';
	if(strncmp("0", buffer, 1) == 0)
	{
		down_time = (unsigned long)time(NULL);
		press_down = 1;
	}
	if(strncmp("1", buffer, 1) == 0)
	{
		...
		if(webupgrade != 1)
		{
			up_time = (unsigned long)time(NULL);
			if( press_down && (up_time - down_time < RESET2DEF_TIMEVAL))
			{
				close(gpiointr_fd);
				gpiointr_fd = -1;
				system("/sbin/ledcontrol -n power -c amber -s off");
				system("/sbin/ledcontrol -n test -c amber -s off");
				printf("Reset Target...\n");
				system("/sbin/reboot");
			}
			else if(press_down && (up_time - down_time >= RESET2DEF_TIMEVAL))
			{
				close(gpiointr_fd);
				gpiointr_fd = -1;
				system("/sbin/ledcontrol -n power -c amber -s off");
				...
				system("/usr/sbin/nvram set action=99");
			}
		}
	}
}
...
static void push_reset_button_signal(int sig)
{
	...
	FD_ZERO(&fdset);
	FD_SET(gpiointr_fd, &fdset);

    	req_timeout.tv_sec = 0;
    	req_timeout.tv_usec = 0;
	
	if(gpiointr_fd >= 0)
		ret = select(gpiointr_fd + 1, &fdset, NULL, NULL, &req_timeout);
	else
		return;
	...
	if(ret < 0) {
		...
		if(FD_ISSET(gpiointr_fd, &fdset)) {
			int i;
			
			i = read(gpiointr_fd, buffer, sizeof(buffer));
			
			buffer[i] = '\0';
			
			if(strncmp("state ar531xgpio6 1", buffer, sizeof("state ar531xgpio6 1") - 1) == 0)
			{
				down_time = (unsigned long)time(NULL);
				press_down = 1;
			}
			if(strncmp("state ar531xgpio6 0", buffer, sizeof("state ar531xgpio6 0") - 1) == 0)
			{
				/* when device is upgrading, the reset button should inactive. */
				if(webupgrade != 1)
				{
...
static void reboot_signal(int sig)
{
	...
	/* I don't know why the message is very slow on DX-K34VWG2-R (IKANOS) 
	   do I use printf instead of message */
...
//tim 
static void call_detcable()
{
	system("/bin/sh /sbin/check_memory_status.sh");
}

//dvd.chen, add for handling reset_default signal from kernel.
static void call_restore_defaults()
{
	system("nvram set restore_defaults=1");
	system("nvram set action=99"); //this cmd will cause to excute monitor.sh
}
...
/*kenny.w added*/		
	signal(SIG_PHY, call_detcable);
/*end of kenny.w*/
	//added by dvd.chen, for handling reset_default signal from kernel.
	signal(SIG_RST_DFT, call_restore_defaults);
	...
	/* 
		...
		when device is upgrading, the reset button should inactive.
	*/
	signal(SIGUSR1, set_webupgrade_signal);
#ifdef DNI_RESET_BUTTON_HDL
	signal(SIGUSR2, push_reset_button_signal);
#else
...
#ifdef DNI_RESET_BUTTON_HDL
    /* first need to open dev, it will register irq request */
	gpiointr_fd = open("/dev/" DEV_NAME_GPIO, O_RDONLY | O_NONBLOCK);
        if(gpiointr_fd < 0) {
		printf("Error opening %s device %d\n", DEV_NAME_GPIO, gpiointr_fd);
		gpiointr_fd = 0;
	}
#endif
