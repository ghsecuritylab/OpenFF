...
static void pass_back(struct dhcpMessage *p, int packet_len, int *fds)
{
    ...
    item->ip.sin_family=PF_INET;                /*hardcode*/
    item->ip.sin_port=htons(68);                  /*hardcode*/
	...
	while (!dhcprelay_stopflag) {
		...
		if (select(max_socket + 1, &rfds, NULL, NULL, &tv) > 0) {
			...
			if (FD_ISSET(fds[0], &rfds)) {
				packlen = get_packet(&dhcp_msg, fds[0]);
				if (packlen > 0) {
			...
			for (i = 1; i < num_sockets; i++) {
                ...
                int nullindex, nullarp;
				if (read_interface(clients[i-1], &nullindex, &dhcp_msg.giaddr, &nullarp) < 0)
...
int dhcprelay_main(int argc, char **argv)
{
    ...
    int nullindex, nullarp;
	if (read_interface(argv[2], &nullindex, &gw_ip, &nullarp) == -1)
		return 1;
