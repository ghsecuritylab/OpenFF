...
int sendOffer(struct dhcpMessage *oldpacket)
{
	...
	if ((lease = find_lease_by_chaddr(oldpacket->chaddr)) && !check_by_arp(lease->yiaddr)) {
	...
	if (!add_lease(packet.chaddr, hostname, packet.yiaddr, server_config.offer_time)) {
...
int sendACK(struct dhcpMessage *oldpacket, uint32_t yiaddr)
{
	...
	//timer = time(0);
	for (i=0,j=0; i < 6; i++) {
		j += sprintf(chaddr+j, "%02x", packet.chaddr[i]);
		if (i != 5) {
			j += sprintf(chaddr+j, ":");
		}
	}
	...
	hostname = get_option(oldpacket, DHCP_HOST_NAME);
	add_lease(packet.chaddr, hostname, packet.yiaddr, lease_time_align);
