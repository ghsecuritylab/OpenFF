...
int read_config(const char *file)
{
	...
	while (fgets(buffer, READ_CONFIG_BUF_SIZE, in)) {
		...
		strcpy(orig, buffer);
#endif
		/* marked, so that we can put # char into dhcp domain name */
	...
	for (i = 0; i < server_config.max_leases; i++)
	{
	   if (leases[i].yiaddr) 
	   {
			...
			if (mac14==0 && mac56==0)
				continue;
#if 0            
            if (arpping(leases[i].yiaddr, server_config.server, 
			...
			if (leases[i].hostname)
				fprintf(fp,"%s ", leases[i].hostname);
			else 
				fprintf(fp,"N/A ");
...
void read_leases(const char *file)
{
	...
	while (i < server_config.max_leases && (fread(&lease, sizeof lease, 1, fp) == 1)) {
		...
		if (lease.yiaddr >= server_config.start && lease.yiaddr <= server_config.end) {
			...
			if (!(add_lease(lease.chaddr, lease.hostname, lease.yiaddr, lease.expires))) {
	...
	LOG(LOG_INFO, "Read %d leases", i, file);
