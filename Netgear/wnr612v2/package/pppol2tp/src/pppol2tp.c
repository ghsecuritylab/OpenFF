...
#define DNI_USE_SLAB_CACHE 0
#define FASTER_XMIT	1
...
#define PRINTK(_mask, _type, _lvl, _fmt, args...)			\
	do {								\
			printk("PPPOL2TP: " _fmt, ##args);		\
	} while(0)
#endif  //#if 0
#else
#define PRINTK(_mask, _type, _lvl, _fmt, args...) do { } while(0)
#endif
...
#else
#define ENTER_FUNCTION	 printk("PPPOL2TP: --> %s\n", __FUNCTION__)
#define EXIT_FUNCTION	 printk("PPPOL2TP: <-- %s\n", __FUNCTION__)
#endif
#else
...
#if (DNI_USE_SLAB_CACHE)
static kmem_cache_t *msghdr_cachep;
static kmem_cache_t *iovec_cachep;
static kmem_cache_t *pppol2tp_send_cachep;
static kmem_cache_t *pppol2tp_tunnel_cachep;
static kmem_cache_t *pppol2tp_session_cachep;
#endif
				  ...
				  struct msghdr *msg, int total_len)
{
	...
	DPRINTK(session->debug, "%s: %s: returning result %d\n", __FUNCTION__,
		session->name, error);
#if (DNI_USE_SLAB_CACHE)
	kmem_cache_free(iovec_cachep, msg->msg_iov);
	kmem_cache_free(msghdr_cachep, msg);
#else
	kfree(msg->msg_iov);
	kfree(msg);
#endif
			    ...
			    size_t total_len)
{
	 ...
	 */
#if (DNI_USE_SLAB_CACHE)
	msg = kmem_cache_alloc(msghdr_cachep, GFP_ATOMIC);
#else
...
#if (DNI_USE_SLAB_CACHE)
	msg->msg_iov = kmem_cache_alloc(iovec_cachep, GFP_ATOMIC);
#else
...
#if (DNI_USE_SLAB_CACHE)
	kmem_cache_free(iovec_cachep, send->iov);
	kmem_cache_free(msghdr_cachep, send->msg);
	kfree_skb(send->skb);
	kmem_cache_free(pppol2tp_send_cachep, send);
#else
	kfree(send->iov);
	kfree(send->msg);
	kfree_skb(send->skb);
...
#if (DNI_USE_SLAB_CACHE)
	send = kmem_cache_alloc(pppol2tp_send_cachep, GFP_ATOMIC);
#else
...
static int pppol2tp_xmit(struct ppp_channel *chan, struct sk_buff *skb)
{
	...
	if (session == NULL)
		goto abort;
#endif
	//SOCK_2_SESSION(sk, session, error, -EBADF, abort, 0);
	...
	if (tunnel == NULL)
		goto abort_put_sess;
#endif
	//SOCK_2_TUNNEL(sk_tun, tunnel, error, -EBADF, abort_put_sess, 0);
	...
	headroom = 16 + sizeof(struct iphdr) +
...
//printk("##%s::src[%08x:%d], dst[%08x:%d]\n", __FUNCTION__, ntohl(inet->saddr), 
//	ntohs(inet->sport), ntohl(inet->daddr), ntohs(inet->dport));
	...
	//added by dvd.chen, copied from ip_queue_xmit, for we need the srcaddr only existed in rt cache. 
	rt = (struct rtable *) skb->dst;
	if (rt == NULL)
	{
		struct sock *sk2 = skb->sk;

		PRINTK(session->debug, PPPOL2TP_MSG_DATA, KERN_DEBUG, "rt cache is NULL\n", __FUNCTION__);
		/* Make sure we can route this packet. */
		rt = (struct rtable *)__sk_dst_check(sk2, 0);
		if (rt == NULL) {
			u32 daddr;
			...
			{
				struct flowi fl = { .oif = sk2->sk_bound_dev_if,
						    .nl_u = { .ip4_u =
							      { .daddr = daddr,
								.saddr = inet->saddr,
								.tos = RT_CONN_FLAGS(sk2) } },
						    .proto = sk2->sk_protocol,
						    .uli_u = { .ports =
							       { .sport = inet->sport,
								 .dport = inet->dport } } };
				 ...
				 * itself out.
				 */
				if (ip_route_output_flow(&rt, &fl, sk2, 0))
				{
					printk("##PPPOL2TP::%s:: ip_route_output_flow failed\n", __FUNCTION__);
					//goto no_route;
					ip_rt_put(rt);  //ref from pptp.c
					
					IP_INC_STATS(IPSTATS_MIB_OUTNOROUTES);
					goto abort_put_sess_tun;
				}
			}
			sk_setup_caps(sk2, &rt->u.dst);
		}
...
//modified by dvd.chen
	...
	if (sk_tun->sk_no_check == UDP_CSUM_NOXMIT)
		...
		skb->csum_start = skb->h.uh - skb->head;
		skb->csum_offset = offsetof(struct udphdr, check);
		uh->check = ~csum_tcpudp_magic(inet->saddr, inet->daddr,
					       udp_len, IPPROTO_UDP, 0);
	}
#else
	uh->check = csum_tcpudp_magic(inet->saddr, inet->daddr, udp_len, 
					IPPROTO_UDP,csum_partial((char *)uh, udp_len, 0));
...
#if 1  //mark off for test, don't send the skb down, just free it
//PRINTK(session->debug, PPPOL2TP_MSG_DATA, KERN_DEBUG, "##calling ip_queue_xmit\n");
...
abort:
	...
	//printk("##PPPOL2TP::%s:: calling kfree_skb\n", __FUNCTION__);
	kfree_skb(skb);
	return 1;
}
#endif
...
#if (DNI_USE_SLAB_CACHE)
	kmem_cache_free(pppol2tp_tunnel_cachep, tunnel);
#else
	kfree(tunnel);
#endif
...
static void pppol2tp_session_destruct(struct sock *sk)
{
	...
	if (session != NULL)
#if (DNI_USE_SLAB_CACHE)
	kmem_cache_free(pppol2tp_session_cachep, session);
#else
		kfree(session);
#endif
...
static int pppol2tp_release(struct socket *sock)
{
	...
	skb_queue_purge(&sk->sk_receive_queue);
	skb_queue_purge(&sk->sk_write_queue);
	//added by dvd.chen, seems that skb was not been freed here
#if 0
	if (session != NULL) {
		struct sk_buff *skb;
						   ...
						   u16 tunnel_id, int *error)
{
	 ...
	 */
#if (DNI_USE_SLAB_CACHE)
	sk->sk_user_data = tunnel = kmem_cache_alloc(pppol2tp_tunnel_cachep, GFP_KERNEL);
#else
		     ...
		     int sockaddr_len, int flags)
{
	 ...
	 */
#if (DNI_USE_SLAB_CACHE)
	session = kmem_cache_alloc(pppol2tp_session_cachep, GFP_KERNEL);
#else
...
//added by dvd.chen, for daddr and dport
#if (FASTER_XMIT)
...
#if (DNI_USE_SLAB_CACHE)
//printk("##%s::size of msghdr[%d], iovec[%d], pppol2tp_send[%d], pppol2tp_tunnel[%d], pppol2tp_session[%d]", __FUNCTION__, 
//	sizeof(struct msghdr), sizeof(struct iovec), sizeof(struct pppol2tp_send), sizeof(struct pppol2tp_tunnel), sizeof(struct pppol2tp_session));
	msghdr_cachep = kmem_cache_create("msghdr", sizeof(struct msghdr), sizeof(struct msghdr), SLAB_HWCACHE_ALIGN, NULL, NULL);	
	if (msghdr_cachep == NULL) 
	{		
		printk(KERN_ERR "Unable to create msghdr slab cache!\n");		
		err = -1;
		goto out;
	}

	iovec_cachep = kmem_cache_create("iovec", 3*sizeof(struct iovec), 3*sizeof(struct iovec), SLAB_HWCACHE_ALIGN, NULL, NULL);	
	if (iovec_cachep == NULL) 
	{		
		printk(KERN_ERR "Unable to create iovec slab cache!\n");		
		err = -1;
		goto out;
	}

	pppol2tp_send_cachep = kmem_cache_create("pppol2tp_send", sizeof(struct pppol2tp_send), sizeof(struct pppol2tp_send), SLAB_HWCACHE_ALIGN, NULL, NULL);	
	if (pppol2tp_send_cachep == NULL) 
	{		
		printk(KERN_ERR "Unable to create pppol2tp_send slab cache!\n");		
		err = -1;
		goto out;
	}

	pppol2tp_tunnel_cachep = kmem_cache_create("pppol2tp_tunnel", sizeof(struct pppol2tp_tunnel), sizeof(struct pppol2tp_tunnel), SLAB_HWCACHE_ALIGN, NULL, NULL);	
	if (pppol2tp_tunnel_cachep == NULL) 
	{		
		printk(KERN_ERR "Unable to create pppol2tp_tunnel slab cache!\n");		
		err = -1;
		goto out;
	}

	pppol2tp_session_cachep = kmem_cache_create("pppol2tp_session", sizeof(struct pppol2tp_session), sizeof(struct pppol2tp_session), SLAB_HWCACHE_ALIGN, NULL, NULL);	
	if (pppol2tp_session_cachep == NULL) 
	{		
		printk(KERN_ERR "Unable to create pppol2tp_session slab cache!\n");		
		err = -1;
		goto out;
	}
#endif
...
#endif
#if (DNI_USE_SLAB_CACHE)
	kmem_cache_destroy(msghdr_cachep);
	kmem_cache_destroy(iovec_cachep);
	kmem_cache_destroy(pppol2tp_send_cachep);
	kmem_cache_destroy(pppol2tp_tunnel_cachep);
	kmem_cache_destroy(pppol2tp_session_cachep);
#endif
}
